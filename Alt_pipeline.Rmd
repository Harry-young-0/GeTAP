...
```{r,Phewas searching }
# Lines for generating the batch ids
# LUT =gwasinfo()
# LUT2 = LUT %>% separate(id, c("ID1", "ID2", "ID_extra")) %>%
#   unite("id_new", ID1:ID2, remove = FALSE,sep = "-")
# unique(LUT2$id_new)

 # Part run test:
# if(file.exists(paste0(file_directory,"/", gene_name,"_test/Final_data/",gene_name,"_GtextoPHEWAS_RAW.csv"))){ # output from the phewas search
#  test = read_csv(paste0(file_directory,"/", gene_name,"_test/Final_data/",gene_name,"_GtextoPHEWAS_RAW.csv"))
#  test_unique_snps = unique(test$rsid)
# }
# 
# 
# if(file.exists(paste0(file_directory,"/", gene_name,"_test/LDMatrix/",gene_name, "_LIMITED_SNP_LIST.csv"))){# if unqiue snps >700
#   unique_Gtex_snps=read_csv(paste0(file_directory,"/", gene_name,"_test/LDMatrix/",gene_name, "_LIMITED_SNP_LIST.csv"))
#   unique_Gtex_snps = as.data.frame(unique(unique_Gtex_snps$rsid))
# }
# 
# 
# if(file.exists(paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps.csv"))){
#   unique_Gtex_snps=read_tsv(paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps.csv"),col_names = F )
# }
# 
# 
# unique_Gtex_snps$test = unique_Gtex_snps$X1 %in% test_unique_snps
# unique_Gtex_snps = unique_Gtex_snps %>% filter(test==F)
...
unique_Gtex_snps2=unique_Gtex_snps

rm(list = ls(pattern = "Gtex_PHEWAS_temp_"))
if(exists("completed_snps")){# add in catch for if completed SNPs is enough?
  if(length(completed_snps)<length(unique_Gtex_snps2)){
  print(paste0("Already attempted run, ",length(completed_snps)," rsids searched - carrying on from where we left off"))
  m=(length(na.omit(completed_snps))%/%batch_size+1)
  unique_Gtex_snps_df = unique_Gtex_snps_df_storage
  unique_Gtex_snps_df$test = !unique_Gtex_snps2 %in%completed_snps
  unique_Gtex_snps_df = unique_Gtex_snps_df %>% filter(test==T)
  unique_Gtex_snps = unique_Gtex_snps_df$rsid
  for(n in 1:(length(na.omit(unique_Gtex_snps))%/%batch_size + 1)){
    test = na.omit(unique_Gtex_snps[(((n-1)*batch_size)+1):(n*batch_size)])
    print(paste0("Temporary dataframe ",n+m-1))
    
    # x=length(mget(ls(pattern = "Gtex_PHEWAS_temp_"), envir = .GlobalEnv))+1 # needs rewokring reomve all gtex_phewas_temp_n after run!
    ...
    if(length(Gtex_PHEWAS_2)==12){
      if(nrow(Gtex_PHEWAS_2)>0){
          ...
          rm(list = ls(pattern = "Gtex_PHEWAS_temp_")) # Clears out all old dataframes to track progress
      }
    }else{break}  
  }
}}else{
  for(n in 1:(length(na.omit(unique_Gtex_snps))%/%batch_size + 1)){
    ...
    if(n==1){
      ...
      for(id in 1:length(batch_for_phewas)){
        ...
        Sys.sleep(10)
        Gtex_PHEWAS =mget(ls(pattern = "Gtex_PHEWAS_temp_"), envir = .GlobalEnv)
        Gtex_PHEWAS = bind_rows(Gtex_PHEWAS)
      }
...
# Then save using the following line:
  Gtex_PHEWAS=unique(Gtex_PHEWAS) # clear up any duplication events
...
```
### BACKUP OF ABOVE LOOP INCASE I BREAK IT:
```{r,Phewas searching }
# Lines for generating the batch ids
# LUT =gwasinfo()
# LUT2 = LUT %>% separate(id, c("ID1", "ID2", "ID_extra")) %>%
...
# 
# 
# if(exists("LIMITED_SNP_LIST")){
#  unique_Gtex_snps = unique(LIMITED_SNP_LIST$rsid)
#  unique_Gtex_snps2 = unique(LIMITED_SNP_LIST$rsid)
#  unique_Gtex_snps_df =as.data.frame(unique_Gtex_snps)
# }else{
#   unique_Gtex_snps = unique(df$rsid)
#   unique_Gtex_snps2 = unique(df$rsid)
#   unique_Gtex_snps_df =as.data.frame(unique_Gtex_snps)
# }
# 
# 
...
#   if(length(completed_snps)<length(unique_Gtex_snps2)){
#   print(paste0("Already attempted run, ",length(completed_snps)," rsids searched - carrying on from where we left off"))
#   m=(length(na.omit(completed_snps))%/%3+1)
#   unique_Gtex_snps_df$test = !unique_Gtex_snps %in%completed_snps
#   unique_Gtex_snps_df = unique_Gtex_snps_df %>% filter(test==T)
#   unique_Gtex_snps = unique_Gtex_snps_df$unique_Gtex_snps
#   for(n in 1:(length(na.omit(unique_Gtex_snps))%/%3 + 1)){
#     test = na.omit(unique_Gtex_snps[(((n-1)*3)+1):(n*3)])
#     print(paste0("Temporary dataframe ",n))
#     for(id in 1:length(batch_for_phewas)){
#       print(batch_for_phewas[id])
#       Gtex_PHEWAS_temp = phewas(test[1],
#                                batch = batch_for_phewas[id],
#                                pval = 1e-04)
#       
#       if(nrow(Gtex_PHEWAS_temp>0)){
...
```
### ALTERNATE ONE BY ONE APPROACH SUPER SLOW - BUT MAY BE NEEDED
```{r,Phewas searching ALT, eval=FALSE}
# # Lines for generating the batch ids
# # LUT =gwasinfo()
# # LUT2 = LUT %>% separate(id, c("ID1", "ID2", "ID_extra")) %>%
# #   unite("id_new", ID1:ID2, remove = FALSE,sep = "-")
...
# #   }
# #   Gtex_PHEWAS =mget(ls(pattern = "Gtex_PHEWAS_temp_"), envir = .GlobalEnv)
# #   Gtex_PHEWAS =rbind(to_do$Gtex_PHEWAS_temp_18,to_do$Gtex_PHEWAS_temp_2)
# # }
# 
# if(exists("LIMITED_SNP_LIST")){
#  unique_Gtex_snps = unique(LIMITED_SNP_LIST$rsid)
...
#   print(paste0("Already attempted run, ",length(completed_snps)," rsids searched - carrying on from where we left off"))
#   m=(length(na.omit(completed_snps))%/%2+1)
#   unique_Gtex_snps_df$test = !unique_Gtex_snps %in%completed_snps
#   unique_Gtex_snps_df = unique_Gtex_snps_df %>% filter(test==T)
#   unique_Gtex_snps = unique_Gtex_snps_df$rsid
#   for(n in 1:(length(na.omit(unique_Gtex_snps))%/%2 + 1)){
#     test = na.omit(unique_Gtex_snps[(((n-1)*2)+1):(n*2)])
#     print(paste0("Temporary dataframe ",n))
#     for(id in 1:length(batch_for_phewas)){
#       print(batch_for_phewas[id])
#       Gtex_PHEWAS_temp = phewas(test[1],
