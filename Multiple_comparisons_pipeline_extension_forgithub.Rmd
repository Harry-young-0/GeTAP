...
# Step 1: Setup of file directories and gene list
Modify Line 24 with a custom list of genes that you have run or "All" to find everything. Importantly, This requires your folders being named exactly what the pipeline created, GENE_test.
FOR GITHUB: Modify line 24 of the ADD NAME HERE.rmd file to genes_for_heatmap = c("GENE_1","GENE_2","GENE_3") - where GENE_1 to GENE_3 are genes you have run through the pipeline and exist in your file directory as GENE_test. OR to genes_for_heatmap = c("All") to find all genes you have run.
...
rm(list=ls())
# Edit the line below for a custom list of genes:
genes_for_heatmap = c("GENE_1","GENE_2","GENE_3")# OR:
#genes_for_heatmap = c("All")
...
file_directory = "/Users/hy14913/Documents/Local_work_folder/JUL_TEST/Heatmaps_test"
...
if(file.exists(paste0(file_directory, "/Heatmaps"))){
  print("Doing nothing as directory is there")
}else{
  dir.create(paste0(file_directory, "/Heatmaps"))
}
...
# list_of_completed_genes=list_of_completed_genes[c(1,2,4,5)]
if(length(genes_for_heatmap)==1){ # if yes then they want all genes done, do first loop- all completed genes
...
}else{# Otherwise use their specified genes
  for(gene in genes_for_heatmap){
    print(gene)
    assign("df_temp", read_csv( paste0(file_directory,"/",gene,"_test/Final_data/final_df.csv")))
    if(exists("df_merge")==T){
      df_merge =rbind(df_temp, df_merge)
    }else{
      df_merge =df_temp
...
file_directory =paste0(file_directory,"/Heatmaps")
write_csv(df_merge, paste0(file_directory,"/df_merge.csv"))
genes_for_heatmap = sort(genes_for_heatmap)
```
...
df_red$`Gene Symbol` =factor(df_red$`Gene Symbol`, levels=c("Legend",genes_for_heatmap))
...
for_legend = Full_trait_database %>% 
  dplyr::select(traitgroup,Subcat,Alt_Subcat) %>%
  unique() %>%
  mutate(`Gene Symbol` = "Legend",
...
#ALTERNATE COLOURSCHEME  based on number of rsids####
df_red$Subcat =ifelse(df_red$Subcat=="Blood Traits", df_red$Alt_Subcat, df_red$Subcat)
...
#ALTERNATE COLOURSCHEME 2  based on proportion of rsids####
df =df_merge
df$Subcat =ifelse(df$Subcat=="Blood Traits", df$Alt_Subcat, df$Subcat)
test_counts = df %>% 
  dplyr::select(trait, traitgroup, Subcat, `Gene Symbol`,rsid, p) %>%
  unique() %>% 
  group_by(trait, traitgroup,Subcat, `Gene Symbol`)%>%
  summarise(subcat_count = n()) %>%
  group_by(`Gene Symbol`)%>%
  mutate(subcat_count.adj=(subcat_count/sum(subcat_count))*100) %>% 
  dplyr::select(`Gene Symbol`,traitgroup,Subcat,subcat_count.adj,subcat_count) %>% 
  group_by(`Gene Symbol`,traitgroup,Subcat)%>%
  summarise(subcat_count.adj = sum(subcat_count.adj),
             subcat_count = sum(subcat_count))
STA_count = df %>% 
  dplyr::select(trait, traitgroup, Subcat,`Gene Symbol`,rsid, p) %>%
  unique() %>% 
  group_by(trait, traitgroup,Subcat, `Gene Symbol`)%>%
  ...
  group_by(`Gene Symbol`) %>%
  summarise(subcat_count= sum(subcat_count))
trait_database_simple = Full_trait_database %>% 
  mutate(Subcat=ifelse(Subcat=="Blood Traits", Alt_Subcat, Subcat))%>%
  dplyr::select(trait,Subcat, traitgroup) %>%
  unique() %>% 
  group_by(Subcat,traitgroup)%>%
  summarise(subcat_count = n()) %>%
  ungroup() %>%
  mutate(subcat_count.adj=(subcat_count/sum(subcat_count))*100) 
trait_database_simple$`Gene Symbol` = "Database"
test_counts=rbind(test_counts, trait_database_simple)

test_counts$traitgroup = factor(test_counts$traitgroup, levels =trait_order)
test_counts$`Gene Symbol` = factor(test_counts$`Gene Symbol`, levels=c("Database" , genes_for_heatmap))
...
ggsave(paste0(file_directory,"/Plots/B4-SubcatvsGene_colby_n.rsid-trait-WITH_DATABASE_COLUMN.pdf"), width=150, height=450, unit ="mm")

test_counts %>%
  ggplot()+
    geom_tile( aes(x=`Gene Symbol`, y=Subcat, fill=subcat_count.adj, height=0.8, width=0.9))+
    geom_text(aes(x=`Gene Symbol`, y=Subcat,label=subcat_count), size=2)+ # Comment in/out for text overlay
    scale_fill_continuous(type = "viridis",trans = log_trans(10), breaks=c(0.01,0.1,1,10,100))+
    theme(axis.text.x = element_text(size=8, angle=45, hjust=1), 
          strip.text.y.right = element_text(angle = 0, face="bold"),
          strip.background.y = element_blank(),
          strip.background.x =element_rect(fill="grey20"),
...
ggsave(paste0(file_directory,"/Plots/B5-SubcatvsGene_colby_n.rsid-trait-rel_to_total_per_column.pdf"), width=150, height=450, unit ="mm")

temp = trait_database_simple %>% 
  dplyr::select(Subcat,traitgroup,subcat_count,  subcat_count.adj)

names(temp) =c("Subcat","traitgroup","subcat_count_database","subcat_count.adj_database" )
temp =left_join(test_counts,temp)
  
temp %>% 
  mutate(subcat_count.adj.rel = subcat_count.adj/subcat_count.adj_database) %>%
  ggplot()+
    geom_tile( aes(x=`Gene Symbol`, y=Subcat, fill=subcat_count.adj.rel, height=0.8, width=0.9))+
    #geom_text(aes(x=`Gene Symbol`, y=Subcat,label=subcat_count.adj.rel, size=2))+ # Comment in/out for text overlay
...
ggsave(paste0(file_directory,"/Plots/B6-SubcatvsGene_colby_n.rsid-trait-rel_to_total_per_subcat_from_database.pdf"), width=150, height=450, unit ="mm")
...
test_counts = df %>% 
  ...
  dplyr::select(`Gene Symbol`,traitgroup,trait_count.adj,trait_count) %>% 
  group_by(`Gene Symbol`,traitgroup)%>%
  summarise(trait_count.adj = sum(trait_count.adj),
             trait_count = sum(trait_count))
STA_count = df %>% 
  dplyr::select(trait, traitgroup, `Gene Symbol`,rsid, p) %>%
  unique() %>% 
  group_by(trait, traitgroup, `Gene Symbol`)%>%
...
trait_database_simple = Full_trait_database %>% 
  ...
  summarise(trait_count = n()) %>%
  mutate(trait_count.adj=(trait_count/sum(trait_count))*100) %>% 
  dplyr::select(traitgroup,trait_count.adj, trait_count) %>% 
  group_by(traitgroup)%>%
  summarise(trait_count.adj = sum(trait_count.adj),
            trait_count = sum(trait_count))
trait_database_simple$`Gene Symbol` = "Database"
...
ggplot()+
 ...
 #SAVE it
ggsave(paste0(file_directory,"/Plots/C2-Summary_bar_proportions.pdf"), width=250, height=130, unit="mm")
```