...
# run these two lines first then comment out
...
For those who have never done any R, hopefully the code is commented well enough to understand but I would highly recommend reading up on basic R syntax before starting. Just google it. There's endless resources.  
...
Finally, it is important to set up a file system similar to what I've done for it to run automatically. /Users/hy14913/Local_work_folder/Epidemiology/GENENAME_test/ containing Gtex and LDMatrix folders, each with Plots folder within. This will make the ctrl-f replace function work well. If you don't want to use your desktop you can ctrl-f and replace "/Users/hy14913/Local_work_folder/Epidemiology/" with whatever head folder you want: e.g. "~/OneDrive - University of Bristol/PhD/Epidemiology/" 

Here is an optional chunk to load in all the directories you'll need to run this notebook as is (as above simply change /Users/hy14913/Local_work_folder/Epidemiology/) to whatever your file path is to the master folder:

INCOMPLETE : add a repository for Full_trait_database and Full_Tissue_database (and egene data if needed)


For my databases, I have made them freely available but obviously where you save them to look up, modify and add to is up to you. So use ctrl-f to find all occurences of 'setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Trait and Tissue Groups")' and change to wherever you have saved teh tissue and trait databases.
...
I would suggest setting up a folder to run all of this into, copying it over to backup files as you go, here I've named in BRCA1_test:

e.g, use the following code in every chunk:
```{r}
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
```

Firstly we read in the dataframe of interest and do some initial clean-up steps:

To read in the dataframe first search your gene of interest into Gtex portal. Then click significant single-tissue eQTL and download the csv. Rename the file Gtex_BRCA1_START.
```{r Initial load and clean}

setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
Gtex_BRCA1 = read_csv("Gtex_BRCA1_START.csv")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_excel_csv(df, "BRCA1_Gtex_cleaned.csv")
...
unique_BRCA1_Gtex_snps = unique(df$rsid)
# setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Quantitative and Qualitative SNP lists")
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write.table(unique_BRCA1_Gtex_snps, "unique_BRCA1_Gtex_snps.csv", sep="\t", row.names=FALSE, col.names=FALSE) # saving the file
```
...
#####IMPORTANT BEFORE YOU START: Check the length (number of rows) of your unique_snp_list. if <1000, continue with 2.1 and 2.2 AND skip 2.3. If >1000 move to 2.3 SKIPPING 2.1 and 2.2
##2.1 Assess Linkage disequilibrium (D')
First step is to search your list of snps (unique_BRCA1_Gtex_snps) into LDMatrix to get an interactive plot of linkage disequilibrium in your snp set. https://ldlink.nci.nih.gov/?tab=ldmatrix 
...
if(length(unique_BRCA1_Gtex_snps)<1000){

setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/LDMatrix")
BRCA1_r2 = LDmatrix(unique_BRCA1_Gtex_snps, pop = "EUR", r2d = "r2", token ="f0d7bc798f5c")
write_csv(BRCA1_r2, "BRCA1_r2.csv")



# open up the .csv file from the website as tab-deliminated 
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/LDMatrix")
BRCA1_r2 = read_csv("BRCA1_r2.csv")
head(BRCA1_r2)


for(n in 1:ncol(BRCA1_r2)){#         LOOP to create a df of NA and values above 0.5 - CHANGE THIS CUTOFF TO WHAT YOU WANT
  if(n==1){ # First column is the rsid so skip it using 'next'
    next
  }
  else if(n==2){linked0.5 = BRCA1_r2[which(BRCA1_r2[,n]>=0.5),] # For the first column setup a new df to write to, called 'linked0.5'
    linked0.5 = linked0.5[,c(1,n)]
  }
  else{
    linked0.5_temp =BRCA1_r2[which(BRCA1_r2[,n]>=0.5),] # Each loop you are making a temporary file 'linked0.5_temp', 
...
for(n in 1:ncol(BRCA1_r2)){#         LOOP to create a df of NA and values above 0.5 - CHANGE THIS CUTOFF TO WHAT YOU WANT
  ...
  else if(n==2){linked0.75 = BRCA1_r2[which(BRCA1_r2[,n]>=0.75),] # For the first column setup a new df to write to, called 'linked0.75'
    linked0.75 = linked0.75[,c(1,n)]
  }
  else{
    linked0.75_temp =BRCA1_r2[which(BRCA1_r2[,n]>=0.75),] # Each loop you are making a temporary file 'linked0.75_temp', 
...
rs_overlap_r20.5 = vector(mode="list", length = nrow(BRCA1_r2)) # set up a dictionary style list
names(rs_overlap_r20.5) = names(BRCA1_r2[,2:ncol(BRCA1_r2)]) # add in the names of the snps (rsids)
...
rs_overlap_r20.75 = vector(mode="list", length = nrow(BRCA1_r2)) # set up a dictionary style list
names(rs_overlap_r20.75) = names(BRCA1_r2[,2:ncol(BRCA1_r2)]) # add in the names of the snps (rsids)
...
rm(df, BRCA1_r2, Gtex_BRCA1, linked_test0.5, linked_test0.75, linked0.5, linked0.75, temp, temp0.5D_2, temp0.75D_2, unique_snps, i,n,test)
##Save the environment
save.image("rs_overlap_ALL_BRCA1.RData")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/LDMatrix")
 ...
 library(LDlinkR)
 BRCA1_LIMITED_SNP_LIST = SNPclip(
   unique_BRCA1_Gtex_snps,
   pop = "EUR",
   r2_threshold = "0.5",
   maf_threshold = "0.01",
  token = "f0d7bc798f5c" )
...
BRCA1_LIMITED_SNP_LIST = BRCA1_LIMITED_SNP_LIST[grep("Variant kept.", BRCA1_LIMITED_SNP_LIST$Details),  ] # selects the ones that passed the test
BRCA1_LIMITED_SNP_LIST = as.data.frame(BRCA1_LIMITED_SNP_LIST[,1])
names(BRCA1_LIMITED_SNP_LIST) = "rsid"

write_csv(BRCA1_LIMITED_SNP_LIST, "BRCA1_LIMITED_SNP_LIST.csv")
unique_BRCA1_Gtex_snps = read_csv("BRCA1_LIMITED_SNP_LIST.csv")# , col_names=F
unique_BRCA1_Gtex_snps = unique_BRCA1_Gtex_snps$rsid
}
```
...
save this list as BRCA1_LIMITED_SNP_LIST.csv in BRCA1_test/Gtex
...
IMPORTANT: Here I have left it as the unique_BRCA1_gtex_snps (unique snp list) but you can also use your LIMITED_snp_list (BRCA1_LIMITED_SNP_LIST) from step 2.3 IF YOU USED IT. 
...
# change out list of snps from unique_BRCA1_Gtex_snps to LIMITED_SNP_LIST here depending on choice above
 Gtex_PHEWAS = phewas(unique_BRCA1_Gtex_snps, pval = 1e-04) # Adjustable p-value here #
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_excel_csv(Gtex_PHEWAS, "BRCA1_GtextoPHEWAS_RAW.csv")
}
```
The result of this is seen below with head(BRCA1_GtextoPHEWAS_RAW).
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
BRCA1_GtextoPHEWAS_RAW = read_csv("BRCA1_GtextoPHEWAS_RAW.csv") # load in Phewas output
BRCA1_Gtex_cleaned = read_csv("BRCA1_Gtex_cleaned.csv") #  load in Gtex input

BRCA1_GtextoPHEWAS_RAW = BRCA1_GtextoPHEWAS_RAW %>%
...
# NOT NECERSARY BUT MAKES IT EASIER TO LOOK AT !
 ...
 BRCA1_Gtex_cleaned = BRCA1_Gtex_cleaned[,c( "Gencode Id","Gene Symbol","rsid","a","b", "NES","|NES|","Tissue", "P-Value", "Variant Id","chr_gtex", "position_gtex", "build_gtex")]
 head(BRCA1_Gtex_cleaned)
 BRCA1_GtextoPHEWAS_RAW = BRCA1_GtextoPHEWAS_RAW[,c("rsid", "ea","nea","trait","beta", "p","se","n","eaf","chr_phewas","position_phewas", "id")] # "traitgroup",
 head(BRCA1_GtextoPHEWAS_RAW)
```
...
test = merge(BRCA1_Gtex_cleaned, BRCA1_GtextoPHEWAS_RAW, all=T, by = "rsid")
# OR
test2 = full_join(BRCA1_Gtex_cleaned, BRCA1_GtextoPHEWAS_RAW, by = "rsid")
...
# test_rsid = PS_BRCA1[which(PS_BRCA1$rsid =="rs78616393"),]
# test_rsid = Gtex_BRCA1[which(Gtex_BRCA1$rsid =="rs78616393"),]

rm(test2) # cleanup removing the extra table 

  
#setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Cross-Reference/GtextoPheWAS-Sorted/BRCA1")
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_excel_csv(test, "BRCA1_merge_GTEX_PHEWAS.csv")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df = read_csv("BRCA1_merge_GTEX_PHEWAS.csv")
...
write_excel_csv(df, "BRCA1_merge_GTEX_PHEWAS-betacor.csv")
```
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df = read_csv("BRCA1_merge_GTEX_PHEWAS-betacor.csv")
...
#setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Cross-Reference/GtextoPheWAS-Sorted/BRCA1")
write_excel_csv(df, "BRCA1_merge_GTEX_PHEWAS_betacor_ambig.csv")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df = read_csv( "BRCA1_merge_GTEX_PHEWAS_betacor_ambig.csv")
...
# save unorganised
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_excel_csv(df, "BRCA1_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv")
```
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df = read_csv("BRCA1_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv")
...
#And finally save it:
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_csv(df, "BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
```
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
unique_BRCA1_Gtex_snps= read_tsv("unique_BRCA1_Gtex_snps.csv", col_names = FALSE) # read the original unique snps file
unique_BRCA1_Gtex_snps = unique_BRCA1_Gtex_snps$X1
if(length(unique_BRCA1_Gtex_snps)<1000){
rm(list=ls())
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
load("rs_overlap_ALL_BRCA1.RData")  # specifically the dataframe with ambiguous data removed
...
df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
backup =df
...
write_csv(test2, "BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_linkagetempr20.5.csv")# full dataframe but with the "test" column 
...
write_csv(test, "BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv") # reduced df
} else{
  df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
  df$test ="filler"
  write_csv(df, "BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
}
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/eGene")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/Gtex_egene")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/Gtex_egene") # SAVE it in dedicated plotting folder
...
## our df plotting
First step is to make sure that you are using the correctly filtered dataframe for no ambiguous or ukb-a data by loading in the ukaORambig_FINAL OR linkage removed set:  BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv dataframe as df:
```{r Reading in the FINAL df}
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
# df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
```
...
First, download the overview of BRCA1 expression in the body from the Gtex search and save this.
...
Gtex_BRCA1_START = read_csv("Gtex_BRCA1_START.csv")

NESvsTiss_full = ggplot(Gtex_BRCA1_START, aes(x=Tissue, y=NES, color = `P-Value`)) + #Plot of SNPs NES vs beta from original dataset
  #geom_jitter(aes(color=NES)) +
  geom_point(position = position_jitterdodge())+ # point plot with jitter to avoid overlap
  scale_colour_viridis_c(direction=-1) + # colourblind friendly palette
  labs(title= "All BRCA1 Expression Changes", x= "Tissue", y= "NES")+ # Lables
  geom_hline(aes(yintercept=0,))+ #  0 line
  theme(axis.text.x = element_text(angle = 70, hjust =1))  # text adjust
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/NES_vs_tissue_and_overview") # save in this dedicated folder
...
NESvsTiss = ggplot(eQTL_from_Phewas, aes(x=Tissue, y=NES, color = `P-Value`)) + #Plot of SNPs NES vs beta
  ...
  labs(title= "BRCA1 Expression Changes Occuring in PheWAS Search", x= "Tissue", y= "NES")+
  geom_hline(aes(yintercept=0,))+
  theme(axis.text.x = element_text(angle = 70, hjust =1)) +
  geom_vline(aes(xintercept=0,))
NESvsTiss

setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/NES_vs_tissue_and_overview")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots") #SAVE it
...
Count_plot = ggplot(df, aes(x=traitgroup, y=tissue_group)) + #Plot of Tissue vs trait for overlap
  geom_count()+
  scale_color_viridis_c()+
  labs(title= "Overall BRCA1 Correlations", paste("Overall Count = ", nrow(df)), x ="Trait Group", y = "Tissue Group")+  #CHANGETHIS
  theme(axis.text.x = element_text(angle = 45, hjust =1))
Count_plot


setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/Tissue_Group_and_Trait_Group_overlap") #SAVE it
...
Count_facet = ggplot(df, aes(x=traitgroup, y=tissue_group)) + #Plot of Tissue vs trait for overlap
  geom_count()+
  scale_color_viridis_c()+
  labs(title= "BRCA1 Correlations by Change in Expression", caption = paste("Overall Count = ", nrow(df), "; Positive Count = ", nrow(df[which(df$NES>0),]), "; Negative Count = ", nrow(df[which(df$NES<0),])), x ="Trait Group", y = "Tissue Group")+  #CHANGETHIS
  theme(axis.text.x = element_text(angle = 45, hjust =1))+ 
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_csv(LD_Gtex, "BRCA1_LDRED_Gtex.csv")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/snp_against_Tissue") #SAVE it
...
Full_tissue_database = read_csv("Full_tissue_database.csv")
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
Gtex = read_csv("BRCA1_Gtex_cleaned.csv")
...
write_csv(Gtex, "BRCA1_Gtex_cleaned.csv") 
...
df_BRCA1_RED = df %>% dplyr::select(Tissue, tissue_group, `P-Value`) 
df_BRCA1_RED = unique(df_BRCA1_RED)
...
df_BRCA1_forcomp = df_BRCA1_RED %>% group_by(Tissue, tissue_group) %>% summarise(count = length(Tissue[!is.na(Tissue)]))

comparison_RNA = left_join(df_BRCA1_forcomp, Gtex_RNA_forcomp, by=c("Tissue", "tissue_group"))
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/snp_against_Tissue") # SAVE it in dedicated plotting folder
...
```{r NO CUTOFF Trait Groups Effected by SNPs that Change in Expression of BRCA1 on Traits by logp}
...
pvsTrait = ggplot(df, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= "-log(p)")+ #labels
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/p vs traitgroup split Expression direction") #SAVE it
...
pvsTrait = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= paste("Number of SNP-Trait \nAssociations"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+  # setting up x axis labels so they don't overlap
...
pvsTrait = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+  # setting up x axis labels so they don't overlap
  #facet_grid(cols = vars(NES_direction)) # facet splits the plot based on a variable, in this case NES direction
  annotate("label",
           x=c(length(unique(df$traitgroup))-0.7,length(unique(df$traitgroup))-0.7),
...
```{r Trait Groups Effected by SNPs that Change in Expression of BRCA1 on Traits}
...
pvsTraitCUTOFF = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of BRCA1 (p<5e-8)" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+  # setting up x axis labels so they don't overlap
  #facet_grid(cols = vars(NES_direction)) # facet splits the plot based on a variable, in this case NES direction
  annotate("label",
...
pvsTrait_bytraitgroup = ggplot(df, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Traits Effected by SNPs that Change in Expression of BRCA1" , x= "Trait by Traitgroup", y= "-log(p)")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/p vs trait split by trait group and Expression direction") #SAVE it
ggsave("pvsTrait_bytraitgroup.pdf", width = 30, height =15)# Actually setting some dimensions due to long names
```
With cutoff:

```{r Traits Effected by SNPs that Change in Expression of BRCA1}
...
pvsTrait_byINFL = ggplot(subcat, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Inflammation Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= "-log(p)")+ #labels
...
pvsTrait_bybody = ggplot(subcat, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  geom_point(position = position_jitterdodge(), size =1)+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Body Fat/Size Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(yintercept=0, alpha=0.3)+# faint line at 0
  geom_hline(yintercept=4, alpha=0.3, color="red")+
  geom_hline(yintercept=-4, alpha=0.3, color="red")+
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
...
pvsTrait_bybody = ggplot() + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  labs(title= "Body Fat/Size Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= "Relative Count")+ #labels
  geom_hline(yintercept=0, alpha=0.3)+# faint line at 0
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
  facet_grid(cols = vars(Subcat), scales="free", space="free_x") # facet splits the plot based on a variable, in this case NES direction
  # annotate("label",
...
pvsTrait_bykidney = ggplot(subcat, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  geom_point(position = position_jitterdodge(), size =1)+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Kidney Trait Groups Effected by SNPs that Change in Expression of BRCA1" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(yintercept=0, alpha=0.3)+# faint line at 0
  geom_hline(yintercept=4, alpha=0.3, color="red")+
  geom_hline(yintercept=-4, alpha=0.3, color="red")+
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
...
```{r Number of SNPs that Change Expression of BRCA1 and Effect a Trait, fig.width=30,fig.height=30}
...
num_of_SNPs_by_beta = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  labs(title= "Number of SNPs that Change in Expression of BRCA1 and Effect a Trait" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+ # setting up x axis labels so they don't overlap
  facet_grid(cols = vars(traitgroup), scales = "free_x", switch = 'x') 
...
First you have to look up the exon table and regulatory features from ensembl. Then bind it to the new merged df and the original BRCA1_Gtex_cleaned df. This step is a little more manual. Unfortunately gene to gene it's hard to write a consistent bit of code for finding introns and exons and cleaning up the df to be what you want it to be. Maybe I can come back here in the final tidying steps! 
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Ensembl/All_Exon_Options")
...
BRCA1_structure =bind_rows(myfiles, .id="id") # merges the list of dataframes into one dataframe

BRCA1_structure = BRCA1_structure[!grepl("stream", BRCA1_structure$`Exon / Intron`),] # remove the 5' and 3' up/downstream regions
BRCA1_structure$EI <- !!rowSums(sapply(BRCA1_structure, grepl, pattern = "ENSE"))
BRCA1_structure$EI = if_else(BRCA1_structure$EI ==TRUE, "Exon", "Intron")

# Finally just making a simple to use mini version of the main table of exons/introns
BRCA1_structure_FIN = BRCA1_structure %>% dplyr::select( Start, End, EI)
BRCA1_structure_FIN = unique(BRCA1_structure_FIN)


setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Ensembl")
write_csv(BRCA1_structure_FIN, "BRCA1_structure_FIN.csv")
write_csv(BRCA1_structure, "BRCA1_structure_FULL.csv")
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Ensembl")
BRCA1_structure_FIN =read_csv("BRCA1_structure_FIN.csv")
BRCA1_structure =read_csv( "BRCA1_structure_FULL.csv")
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
Gtex = read_csv("BRCA1_Gtex_cleaned.csv")
...
limitsSTART = BRCA1_structure[which.max(BRCA1_structure$Start),]
limitsEND = BRCA1_structure[which.min(BRCA1_structure$End),]
limits =rbind(limitsSTART,limitsEND)

exons = BRCA1_structure[which(BRCA1_structure$EI=="Exon"),] # select just exon rows
...
df$Intron_or_Exon = "Intron" # confusingly BRCA1 runs backwards so all the signs are flipped to make sense : START>POS>END
for( n in 1:nrow(df)){
...
Gtex$Intron_or_Exon = "Intron" # confusingly BRCA1 runs backwards so all the signs are flipped to make sense : START>POS>END
for( n in 1:nrow(Gtex)){
  for(i in 1:nrow(exons)){
...
chr = df$chr_phewas[1]
BRCA1_region_st = min(Gtex$position_gtex-10000) # 9640000
BRCA1_region_fin = max(Gtex$position_gtex+10000) #  11400000
filter_values=list(chr,BRCA1_region_st,BRCA1_region_fin)
temp = getBM(attributes=c("chromosome_name", "chromosome_start", "chromosome_end", "feature_type_name"), 
      filters = c("chromosome_name", "start", "end"),
      values = filter_values, 
      mart = ensembl)

write_csv(temp, "BRCA1_full_REG_region.csv")
```
Now we assign those reg features to df and gtex dataframes
```{r}
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Ensembl")
reg_feat = read_csv("BRCA1_full_REG_region.csv") # from biomart from ensembl - 
# setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
# df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv") # from before
...
setwd("~/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df_merge = read_csv("BRCA1_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv")
...
write_csv(df_merge, "BRCA1_merge_GTEX_PHEWAS_betacor_ambig_grouped_reg.csv")
# I'm just overiding this file with the new one for simplicity but you can make a new file if you see fit
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
write_csv(df, "BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
write_csv(Gtex, "BRCA1_Gtex_cleaned_REG.csv")
...
DO THESE NEXT FEW STEPS for BRCA1 KEAP1 ORMDL3 and ATP5f1
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
df_merge = read_csv("BRCA1_merge_GTEX_PHEWAS_betacor_ambig_grouped_reg.csv")
Gtex = read_csv("BRCA1_Gtex_cleaned_REG.csv")
df = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Ensembl")
reg_feat = read_csv("BRCA1_full_REG_region.csv") 
BRCA1_rectangles = read_csv("BRCA1_structure_FIN.csv") # from before
```

Now, importantly, both for the linear and curved gene diagram the leg work remains the same. 
* Define the regions by having start, end and feature columns (as in BRCA1_rectangles). 
...
First, we need to take the regulatory feature dataframe (downloaded form ensembl BioMART - see start of section) and clean it to look like BRCA1_rectangles
```{r Setting up reg feature dataframe }
# Now simplifying the regulatroy region page down
reg_feat_for_bind = reg_feat[,2:4]
# sorting the names to be the same as the BRCA1_rectangles 
...
gene_and_reg = rbind(reg_feat_for_bind, BRCA1_rectangles)

setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Ensembl") # saving in the Ensembl folder
write_csv(gene_and_reg, "BRCA1_and_reg_feat_FOR_GENE_DIAGRAM.csv")
...
gene_structure_G = ggplot()+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= "BRCA1 Gene Region Diagram (Gtex SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
...
setwd("~/Local_work_folder/Epidemiology/BRCA1_test/Gene_diagram_output")
...
gene_structure_RED = ggplot()+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= "BRCA1 Gene Region Diagram (Gtex after Merge SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
gene_structure_RED = ggplot()+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= "BRCA1 Gene Region Diagram (Gtex after Merge and LD removal SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
gene_structure_RED_P = ggplot()+
  ...
  labs(title= "BRCA1 Gene Region Diagram (Gtex-PheWAS SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex/Plots/REG_region_counts") #SAVE it
...
setwd("~/Local_work_folder/Epidemiology/BRCA1_test/Circos")
...
library(tidyverse)
setwd("/Users/hy14913/Local_work_folder/Epidemiology/BRCA1_test/Gtex")
Gtex_RED = read_csv("BRCA1_Gtex_cleaned_REG.csv")
df_final = read_csv("BRCA1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
df =df_final
...
# COMMON ERRORS
'Error: 'BRCA1_Dprime.csv' does not exist' - You need to run this chunk (CHUNK 7 - r Automated r2 and d prime START dataset, eval= FALSE, include=FALSE) of code seperately using the little play symbol. I've done this because it can be a lengthy step and is optional (only use if you have less than 999 uniques SNPs)
