...
This is hopefully going to be a fairly simple and straightforward experience...

That being said there are a few manual steps before you get going. This is also laid out on the github page but also written here for completeness 

The pheWAS search often times out due to busy servers! Please do try running again at another time if it does this, I think evenings/off peak times are better but it seems fairly random.


## To do list:

You'll need to assign a gene_name, file_directory and LD_token in this block of code, making sure each is within "" as in the examples. 

  1. Gene name should be an all caps version that appears in gtex: search your gene on https://gtexportal.org/home/ if you're unsure. e.g "SOD2" = super oxide dismutase 2
  2.File directory should be in the format "DRIVE:/YOUR FILE DIRECTORY". This should be as straight forward as R-click "copy as pathname" in mac finder. In windows you'll have to copy the pathname and change all \ for / (who knows why they have this discrepancy!) e.g: copied windows pathname: "C:\Users\Harry\Documents" would need to be changed to: "C:/Users/Harry/Documents"
  3.LD token can be acquired from: https://ldlink.nci.nih.gov/?tab=apiaccess and they'll email it to you. A simple step to stop bots I think
  4. Finally, the gnomAD data will need to be saved into the gnomAD folder, First, run the pipeline, the website will pop up, click the download csv button (details on the github instructions) and then save it into the gnomad folder created in your file directory by the pipeline.The code will not run until this step is done :) .
  
  5. The packages will install 
...
# e.g: file_directory ="/Users/hy14913/Documents/Local_work_folder/JUL_TEST"

file_directory = "DRIVE:/YOUR FILE DIRECTORY"
file_directory ="/Users/hy14913/Documents/Local_work_folder/JUL_TEST"
...
Need to answer the promt for ieugwasr : "1: ALl"
and biomart "none"
when prompted

INCOMPLETE - WORK AROUND?
ALSO maybe a if required for bioc manager too?
```{r}
...
Now load in and install the final packages:
```{r}
...
write_csv(egene ,paste0(file_directory,"/eGene/FINAL_Gtex_egene.csv"))
  ...
  setwd(paste0(file_directory,"/", gene_name,"_test"))
  subfolder_names <- c("Circos","Ensembl","Gtex","LDMatrix",  "Gene_diagram_output","Gnomad", "Final_data", "Final_plots") 
  for (n in 1:length(subfolder_names)){
    folder =dir.create(paste0(file_directory,"/", gene_name,"_test/",subfolder_names[n]))
  }
  
  setwd(paste0(file_directory,"/",gene_name,"_test/Ensembl"))
  dir.create("All_Exon_Options")
  
  setwd(paste0(file_directory,"/", gene_name,"_test/Gtex"))
  dir.create("Plots")
  dir.create("Final Dataframes")
  setwd(paste0(file_directory, "/",gene_name,"_test/Gtex/Plots"))
  ...
  for (n in 1:length(subfolder_names)){
    folder = dir.create(paste0(file_directory, gene_name,"_test/Gtex/Plots/",subfolder_names[n]))
  }
...
setwd(paste0(file_directory,"/", gene_name,"_test/Gnomad"))
temp = list.files() # list of files within the directory
if(length(temp)==0){
  browseURL(paste0("https://gnomad.broadinstitute.org/gene/",gene_name,"?dataset=gnomad_r3"))
  stop()
}else{
 gnomad = read_csv(temp)
 write_csv(gnomad, paste0("gnomAD_v3.1.2_",gene_name,".csv"))
}
...
paste0(file_directory,"/", gene_name,"_test/Gtex")
...
I calculate linkage at an r2 cutoff of 0.5 and 0.75, this can be chosen between later on, email me if you need help modifying the code!
...
rm(linked_test0.5, linked_test0.75, linked0.5, linked0.75, temp, n,test,Gtex_1,r2,gnomad)
...
unique_Gtex_snps = read_csv(paste0(file_directory,"/", gene_name,"_test/LDMatrix/",gene_name, "_LIMITED_SNP_LIST.csv"))# , col_names=F
unique_Gtex_snps = unique_Gtex_snps$rsid
}
```
...
for(n in 1:round(length(unique_Gtex_snps)/50, digits=0)){
  test = na.omit(unique_Gtex_snps[(((n-1)*50)+1):(n*50)])
...
write_excel_csv(Gtex_PHEWAS, paste0(file_directory,"/", gene_name,"_test/",gene_name,"_GtextoPHEWAS_RAW.csv"))
}
```
...
GtextoPHEWAS_RAW = read_csv(paste0(file_directory,"/", gene_name,"_test/",gene_name,"_GtextoPHEWAS_RAW.csv")) # load in Phewas output
...
test = merge(Gtex_cleaned, GtextoPHEWAS_RAW, all=T, by = "rsid")
# OR
test2 = full_join(Gtex_cleaned, GtextoPHEWAS_RAW, by = "rsid")
...
# alt: read_csv(paste0(file_directory,"/", gene_name,"_test/Gtex/", gene_name, "_merge_GTEX_PHEWAS.csv"))
df=test
...
write_excel_csv(df, paste0(file_directory,"/", gene_name,"_test/Gtex/", gene_name,"_merge_GTEX_PHEWAS-betacor.csv"))
...
df = read_csv(paste0(file_directory,"/", gene_name,"_test/Gtex/", gene_name,"_merge_GTEX_PHEWAS-betacor.csv"))
...
write_excel_csv(df, paste0(file_directory,"/", gene_name,"_test/Gtex/", gene_name,"_merge_GTEX_PHEWAS_betacor_ambig.csv"))
...
df = read_csv( paste0(file_directory,"/", gene_name,"_test/Gtex/", gene_name,"_merge_GTEX_PHEWAS_betacor_ambig.csv"))
...
write_excel_csv(df, paste0(file_directory,"/", gene_name,"_test/Gtex/",gene_name,"_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv"))
```
...
df = read_csv( paste0(file_directory,"/", gene_name,"_test/Gtex/",gene_name,"_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv"))
...
if(length(unique_Gtex_snps)<700){
  rm(list=ls()[! ls() %in% c("file_directory","gene_name","LD_token","unique_Gtex_snps")])
...
write_csv(test2, paste0(gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL_linkagetempr20.5.csv"))# full dataframe but with the "test" column 
...
write_csv(test, paste0(gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")) # reduced df
} else{
  df = read_csv(paste0(gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL.csv"))
  df$test ="filler"
  write_csv(df, paste0(gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv"))
}
...
NOTE: I use gnomad3 as the newest version. There is good reasone to use v2 instead. If this is important to you I can add it in but in general, it makes it much simpler to do only v3.
```{r Initial load and clean gnomad}



Gnomad3_df = read_csv(paste0(file_directory,"/",gene_name,"_test/Gnomad/gnomAD_v3.1.2_",gene_name,".csv"))
Gnomad3_df = Gnomad3_df%>%
  rename(rsid = `rsIDs`)
...
#Also clear up any NA rsids that have snuck through
Gnomad3_df = Gnomad3_df[which(is.na(Gnomad3_df$rsid)==F),]
...
write_excel_csv(Gnomad3_df, paste0(file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_Gnomad3_cleaned.csv"))
...
unique_Gnomad3_snps = unique(Gnomad3_df$rsid)
write.table(unique_Gnomad3_snps, paste0(file_directory,"/",gene_name,"_test/Gnomad/unique_",gene_name,"_Gnomad3_snps.csv"), sep="\t", row.names=FALSE, col.names=FALSE)
...
##2.1 Assess Linkage disequilibrium (r2)
As explained earlier, repeated here for the Gnomad SNPS
...
if(length(unique_Gnomad3_snps)<700){


r2 = LDmatrix(unique_Gnomad3_snps, pop = "EUR", r2d = "r2", token =LD_token)
write_csv(r2, paste0(file_directory,"/",gene_name,"_test/LDMatrix/",gene_name,"_gnomad3_r2.csv"))



# open up the .csv file from the website as tab-deliminated 

r2 = read_csv( paste0(file_directory,"/",gene_name,"_test/LDMatrix/",gene_name,"_gnomad3_r2.csv"))
...
#Cleanup before saving the data
rm(list=ls()[! ls() %in% c("linked_test0.75","linked_test0.5","LD_token","gene_name", "file_directory", "unique_Gnomad3_snps", "rs_overlap_r20.75", "rs_overlap_r20.5")])
##Save the environment
save.image(paste0(file_directory,"/",gene_name,"_test/Gnomad/rs_overlap_ALL_",gene_name,"_gnomad3.RData"))
...
if(length(unique_Gnomad3_snps)>=700){
  ...
  #INCOMPLETE 02/09
chunk <- 4000
n <- length(unique_Gnomad3_snps)
r  <- rep(1:ceiling(n/chunk),each=chunk)[1:n]
d <- split(unique_Gnomad3_snps,r)
library(LDlinkR)
 LIMITED_SNP_LIST = SNPclip(
   d[[1]],
   pop = "EUR",
...
for(n in 2:length(d)){
 ...
 LIMITED_SNP_LIST_temp = SNPclip(
   d[[n]],
   pop = "EUR",
   r2_threshold = "0.5",
   maf_threshold = "0.01",
  token = LD_token )
 LIMITED_SNP_LIST = rbind(LIMITED_SNP_LIST, LIMITED_SNP_LIST_temp)
}
...
write_csv(LIMITED_SNP_LIST, file_directory,"/",gene_name,"_test/LDMatrix/LIMITED_SNP_LIST_gnomad3.csv")
unique_Gnomad3_snps = read_csv("LIMITED_SNP_LIST_gnomad3.csv")# , col_names=F
unique_Gnomad3_snps = unique_Gnomad3_snps$rsid
}
...
for(n in 1:round(length(unique_Gnomad3_snps)/50, digits=0)){
  print(n)
  test = na.omit(unique_Gnomad3_snps[(((n-1)*50)+1):(n*50)])
  if(n==1){
    print("First dataframe")# check it is actually pinging the server
    Gnomad3_PHEWAS = phewas(test, pval = 1e-04) # Adjustable p-value here #
  }
  else{
    print(paste0("Temporary dataframe ",n))
    Gnomad_PHEWAS_2 = phewas(test, pval = 1e-04) # Adjustable p-value here #
    if(nrow(Gnomad_PHEWAS_2)>0){
      Gnomad3_PHEWAS = rbind(Gnomad3_PHEWAS, Gnomad_PHEWAS_2)
    }
  }
...
# Then save using the following line:
write_excel_csv(Gnomad3_PHEWAS,paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadtoPHEWAS_RAW.csv"))
}
...
GnomadtoPHEWAS_RAW = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadtoPHEWAS_RAW.csv")) # load in Phewas output
Gnomad_cleaned = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_Gnomad3_cleaned.csv")) #  load in Gnomad input

GnomadtoPHEWAS_RAW = GnomadtoPHEWAS_RAW %>%
  rename(position_phewas = position, # Renaming columns to keep track of their original dataframe
         chr_phewas = chr)
...
test = merge(Gnomad_cleaned, GnomadtoPHEWAS_RAW, all=T, by = "rsid")
# OR
test2 = full_join(Gnomad_cleaned, GnomadtoPHEWAS_RAW, by = "rsid")
head(test) 
head(test2)

```
The next step is cleaning up the merge and checking that it has worked as expected:
NOTE: the positions WILL NOT MATCH. phewas uses build 37 where gnomad uses b38 so just be aware of this
...
write_excel_csv(test, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_merge_Gnomad_PHEWAS.csv"))
...
df_gnomad = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_merge_Gnomad_PHEWAS.csv"))
...
gtex_snps =  unique(Gtex_cleaned$rsid)
...
df = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_merge_Gnomad_PHEWAS.csv"))
#Sometimes the alleles slip into TRUE/FALSE collumns so to correct that:
...
df %>%dplyr::select (beta, beta_cor, ea, nea, Reference, Alternate)
...
write_excel_csv(df, paste0(file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_merge_Gnomad_PHEWAS-betacor.csv"))
```
### 3b. ambiguous cases (AT  CG):
To solve reverse vs sense strand issues
...
write_excel_csv(df, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_merge_Gnomad_PHEWAS_betacor_ambig.csv"))
...
write_excel_csv(df, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_merge_Gnomad_PHEWAS_betacor_ambig_grouped.csv"))
```
...
#And finally save it:
write_csv(df, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL.csv"))
#df = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL.csv"))
```
...
Gnomad_cleaned = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_Gnomad3_cleaned.csv"))
unique_Gnomad_snps= unique(Gnomad_cleaned$rsid)
if(length(unique_Gnomad_snps)<700){
rm(list=ls()[! ls() %in% c("file_directory","gene_name","LD_token","unique_Gnomad_snps","df")])
load(paste0(file_directory,"/",gene_name,"_test/Gnomad/rs_overlap_ALL_",gene_name,"_gnomad3.RData"))  # specifically the dataframe with ambiguous data removed
...
test2 = test2 %>% rename(beta_direction = beta_direction.y)
test2 = test2 %>% dplyr::select(-c("beta_direction.x"))
write_csv(test2, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL_linkagetempr20.5.csv"))# full dataframe but with the "test" column 
...
write_csv(test, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")) # reduced df
} else{
  df = read_csv(paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL.csv"))
  df$test ="filler"
  write_csv(df, paste0( file_directory,"/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv"))
}
...
df_gtex = read_csv(paste0(file_directory, "/",gene_name,"_test/Gtex/", gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv"))
Gtex_cleaned = read_csv(paste0(file_directory, "/",gene_name,"_test/Gtex/",gene_name,"_Gtex_cleaned.csv"))
Full_tissue_database= read_csv(paste0(file_directory, "/Trait and Tissue Groups/Full_tissue_database.csv"))
Full_trait_database= read_csv(paste0(file_directory, "/Trait and Tissue Groups/Full_trait_database.csv"))
Gtex_cleaned =left_join(Gtex_cleaned, Full_tissue_database) # assigning tissue groups

df_gnomad = read_csv( paste0(file_directory, "/",gene_name,"_test/Gnomad/",gene_name,"_GnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv"))


```
To wean out SNPs that are actually GTEx, we can join the gnomAD final dataset (df_gnomad) with the original GTEx dataframe (Gtex_cleaned). This means any SNPs with a value in the Tissue column is actually a GTEx SNP.

###Merge of Original Gtex and df_gnomad
```{r Merge of Original Gtex and df_gnomad}
Gtex_cleaned_short = Gtex_cleaned %>% dplyr::select(Tissue, tissue_group, tissue_group_secondary,rsid)
test =left_join(df_gnomad, Gtex_cleaned_short, by="rsid")# Joining by rsid
test =test[which(is.na(test$Tissue)==T),] # find all rows with NA for tissue (non Gtex SNPS)
test2 = intersect(df_gnomad$rsid, Gtex_cleaned$rsid) # GTEx SNPs within gnomAD dataframe.
...
if(identical(setdiff(test4,test3), test2)==T){
  if(length(test2)>1){
    print( "All Gtex SNPs succesfully removed from df_gnomad, the unique gnomad snps are: ")
    paste0(test2)
  }else{
  paste0( "All Gtex SNPs succesfully removed from df_gnomad, the unique gnomad snp is: ", test2)
  }
}
df_gnomad_full =df_gnomad
df_gnomad=test
df_gnomad$`Gene Symbol` = gene_name
...
write_csv(LD_Gtex, paste0(file_directory,"/", gene_name,"_test/Gtex/LDRED_Gtex.csv")) # Storage of this dataframe for future use
...
SNPcountperTissuegroup = ggplot(test, aes(x=tissue_group, y=count/nrow(LD_Gtex), fill=NES_direction)) + 
  ...
  #ylim(0,0.5)+
  scale_fill_viridis_d(begin=1, end=0, name=paste0("Expression Change")) + # I like this palette but you can remove it or change it 
  labs(title= "Proportion of SNP - Trait Associations per Tissue" , x= "Tissue Group", y=paste("Proportion of rsIDs \n(n/total)"))+ #labels
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/TissueGROUP-rsid_count_OVERVIEW.pdf"), width= 150, height=100, unit="mm")


```


#### C - Correlation between RNA-seq/genotyped merged df - Checking the pattern isn't just beecause of number of samples Gtex has taken from each tissue:
```{r Plotting the raw Gtex egene data against Tissue line graph}

df_RED = df %>% dplyr::select(Tissue, tissue_group, `P-Value`) 
df_RED = unique(df_RED)


Gtex_egene = read_csv(paste0(file_directory,"/eGene/FINAL_Gtex_egene.csv"))
...
df_forcomp = df_RED %>% group_by(Tissue, tissue_group) %>% summarise(count = length(Tissue[!is.na(Tissue)]))
# Sorting out annoying vcolumn name mismatch
df_forcomp = left_join(df_forcomp, Full_tissue_database)
df_forcomp =df_forcomp %>% rename(Tissue_old =Tissue,
                                  Tissue =Tissue_nice)

comparison_RNA = left_join(df_forcomp, Gtex_RNA_forcomp, by=c("Tissue", "tissue_group"))
comparison_RNA =comparison_RNA %>% filter(!is.na(Tissue))

rsq <- function (x, y) cor(x, y) ^ 2
r2 = rsq(comparison_RNA[,3],comparison_RNA[,6])

Correlation = ggplot(comparison_RNA, aes(x=comparison_RNA$count, y=comparison_RNA$RNASeq_and_Genotyped_Samples, color = Tissue)) + #Plot of SNPs NES vs beta from original dataset
...
Correlation_egene = Correlation + annotate(geom="text",  x=12, y=20, label = paste0("r2 = ", round(r2[1],4)),
              fill="black")
Correlation_egene



ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/RNAvsMERGEDdfcount.pdf"), width= 150, height=100, unit="mm")


```
#### C-2 - Correlation between nmuber of SNPs in each trait group compared to trait database category size - Checking the pattern isn't just beecause of this:
```{r Plotting the raw Gtex egene data against Tissue line graph}
...
rsq <- function (x, y) cor(x, y) ^ 2
r2 = rsq(comparison_TG[,2],comparison_TG[,3])

Correlation = ggplot(comparison_TG, aes(x=count, y=count_full, color = traitgroup)) + #Plot of SNPs NES vs beta from original dataset
  #geom_jitter(aes(color=NES)) +
  geom_point()+ 
  #geom_text(data=subset(comparison_egene, eGenes/comparison_egene$count>45 | eGenes/comparison_egene$count<20), aes(label=Tissue), vjust=-0.6, hjust=0.8)+
   # colourblind friendly palette
  scale_color_manual(values = grid_col) +
  labs(title= paste0("Correlation of traitgroup from our df to full database"), x= "Number of SNP-Traitgroup Associations", y= "Number of Possible Traits in Traitgroups") +
  #theme(legend.position = "none")+ 
  geom_line(inherit.aes = TRUE, data = comparison_TG[,2:3], stat="smooth", method = "lm", formula = y~x, alpha = 0.5, color = "black")+
    theme(axis.text.x = element_text(angle = 45, hjust =1),
         #panel.background = element_rect(fill="white"),
         #panel.grid.major.x = element_blank(),
         #panel.grid.major.y = element_line(colour = "gray90", linetype="solid", size=0.4),
...
Correlation_tg = Correlation + annotate(geom="text",  x=40, y=15000, label = paste0("r2 = ", round(r2[1],4)),
              fill="black")
Correlation_tg



ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/TG_dfcount_cor.pdf"), width= 150, height=150, unit="mm")
...
Position_count_OVERVIEWd1 = ggplot(LD_Gtex, aes(x=position_gtex,  fill = NES_direction)) + 
  ...
  labs(title= paste0("Number of tissues each SNP Affects expression in", gene_name) , x= paste0("Position (Chr ",Chromosome, ")"), y = "Number of Tissues")+ #labels
  theme(legend.position = "none",
         panel.background = element_rect(fill="white"),
         panel.grid.major.x = element_blank(),
...
Position_count_OVERVIEWd1

LD_Gtex$position_gtex = as.numeric(LD_Gtex$position_gtex)

ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/d1-Position_count_OVERVIEW.pdf"), width= 60, unit="mm")
```

####E0 Overview of trait-SNP associations per traitgroup (incl empty ones)
```{r}
...
SNP_by_traitgroupe0= ggplot(summary_of_df, aes(x=reorder(traitgroup, -traitgroup_count), y=traitgroup_count, fill= traitgroup))+
  geom_col()+
...
SNP_by_traitgroupe0
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/e0-ntraitgroup_associations.pdf"), width=200,height=120, unit="mm")
...
temp$traitgroup = factor(temp$traitgroup, levels =c("Body Fat/Size",
            "Metabolism (inc. Diabetes)",
            "Tumours (Benign/Cancer)",
            "Atrophy/Dystrophy", 
            "Congenital Defects", 
...
pvalvstraitgroupe1 = ggplot(temp, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
...
#  gghighlight(TOP5=="TOP5")+
  labs(title= paste0("Trait Groups Affected by SNPs that Change in Expression of ",gene_name) , x= "Trait Group", y= "-log(p) x Direction of expression change")+ #labels
  ...
  #ylim(-200,200)+
  # annotate("label",
  #          x=c(length(unique(temp$traitgroup))+1,length(unique(temp$traitgroup))+1),
  #          y=c(max(temp$logp_DIR),min(temp$logp_DIR)),
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/e1pvalvstraitgroup.pdf"), width=200, height =100 , units="mm")
```


#####E2 Without Other or OGEC - often skews data due to high amount of SNPS in these categories
...
pvalvstraitgroupNO_O_e2 = ggplot(temp, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  # geom_jitter(aes(color=NES)) + # Jitter can be nicer but i think i prefer point with jitterdodge
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= paste0("Trait Groups Affected by SNPs that Change in Expression of ", gene_name, " (No Other or Other Gene Expression Change)") , x= "Trait Group", y= "-log(p) x Direction of expression change")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
theme(axis.text.x = element_text(angle = 60, hjust =1, size = 8),
        axis.text.y =   element_text(size = 10),
        axis.title = element_text(size = 10, face="bold"),
...
pvalvstraitgroupNO_O_e2


ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/e2-pvalvstraitgroup_noother.pdf"), width=200, height =100 , units="mm")
```
...
OGECe3 = ggplot(temp, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  # geom_jitter(aes(color=NES)) + # Jitter can be nicer but i think i prefer point with jitterdodge
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs( x= "Genes", y= "-log(p) * Direction")+ #labels title= paste0("Genes Affected by SNPs that Change in Expression of ", gene_name) ,
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
  ...
  #ylim(-200,200)+
  # annotate("label",
  #          x=c(length(unique(temp$trait))/2,length(unique(temp$trait))/2), 
  #          y=c(max(temp$logp_DIR)-(0.1*max(temp$logp_DIR)),min(temp$logp_DIR)-(0.1*min(temp$logp_DIR))),
  #          label=c("Positive","Negative"), 
  #          colour = c("white","white"), 
  #          fill =c("darkorchid4","darkorchid4"))+
...
filename=paste0(file_directory,"/",gene_name,"_test/Final_plots/e3-pvalvstraitgroup_ogec.pdf")
...
#### E4 Top 5 categories after other and OGEC: AUG 22 - PLOT OF WHICH IS HIGHEST GROUPS, TOP £ INTO INDIVIDUAL PLOTS - THEN WORK OUT HOW TO DISPLAY ALL NICELY
```{r, fig.width=6, fig.height=10}
# Top5_traitgroups = summary_of_df %>% filter(traitgroup!= "Other/Unspecified", traitgroup!= "Other Gene Expression Change") %>% top_n(5) 
# Top5_traitgroups = arrange(Top5_traitgroups, desc(traitgroup_count))


for(i in 1:5){
  print(i)
  temp =df[which(df$traitgroup ==summary_of_df_no_OGEC$traitgroup[i]),] 
  for_label = df[which(df$traitgroup ==summary_of_df_no_OGEC$traitgroup[i]),]
  for_label = length(unique(for_label$trait))
  
  #HERE mon aug 22
  pvsTrait_bygroup = ggplot(temp, aes(x=trait, y=logp_DIR, color=beta_direction)) + 
    #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
    # geom_jitter(aes(color=NES)) + # Jitter can be nicer but i think i prefer point with jitterdodge
    ...
    coord_flip()+
    force_panelsizes(cols=unit(40, "mm"))#cols=unit(1,"in"))
  pvsTrait_bygroup
  # 67  = 14 by 14
  # 168 = 20 by 14

assign(paste0("e4_", i), pvsTrait_bygroup) 

filename=paste0(file_directory,"/",gene_name,"_test/Final_plots/Number_", i,"_traitgroup.pdf")
  if(for_label>150){
    ggsave(filename, height=560, width=230, unit="mm")
...
temp_pos = df[which(df$NES_direction=="Positive"),]
temp_neg = df[which(df$NES_direction=="Negative"),]

f1 = ggplot()+ #Plot of Traitgroup vs SNP Count
  geom_bar(data= temp_pos, aes(x=factor(traitgroup), fill=beta_direction, y=..count../nrow(temp_pos)),  position = position_dodge2(width = 1, preserve = "single"))+
  geom_bar(data= temp_neg, aes(x=factor(traitgroup), fill=beta_direction, y=(..count../nrow(temp_neg))*-1), position = position_dodge2(width = 1, preserve = "single"))+##  alpha is transparancy from 0-1
    geom_hline(yintercept=0, color ="gray70")+
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= paste0("Trait Groups Effected by SNPs that Change in Expression of", gene_name) , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 60, hjust =1, size = 8),
        axis.text.y =   element_text(size = 10),
        axis.title = element_text(size = 10, face="bold"),
        panel.grid.major.x = element_blank(),
        ...
        panel.grid.minor.y = element_line(colour = "gray90", linetype="solid", size=0.2),
        panel.background = element_rect(fill="white"),
        legend.position = "none")  # setting up x axis labels so they don't overlap
  ...
  #          colour = c("white","white"), 
  #          fill =c("darkorchid4","darkorchid4"))
f1 + scale_x_discrete(limits=c("Body Fat/Size",
            "Metabolism (inc. Diabetes)",
            "Tumours (Benign/Cancer)",
            "Atrophy/Dystrophy", 
            "Congenital Defects", 
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/f1COUNTvstraitgroup_REL_overview_SEATLE_all.pdf"), width=200, height =100 , units="mm")
...
temp_pos = df[which(df$NES_direction=="Positive" & df$traitgroup!="Other Gene Expression Change"& df$traitgroup!="Other/Unspecified"),]
temp_neg = df[which(df$NES_direction=="Negative" & df$traitgroup!="Other Gene Expression Change"& df$traitgroup!="Other/Unspecified"),]

f2 = ggplot()+ #Plot of Traitgroup vs SNP Count
  geom_bar(data= temp_pos, aes(x=traitgroup, fill=beta_direction, y=..count../nrow(temp_pos)),  position = position_dodge2(width = 1, preserve = "single"))+
  geom_bar(data= temp_neg, aes(x=traitgroup, fill=beta_direction, y=(..count../nrow(temp_neg))*-1), position = position_dodge2(width = 1, preserve = "single"))+##  alpha is transparancy from 0-1
    geom_hline(yintercept=0, color ="gray70")+
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= paste0("Trait Groups Effected by SNPs that Change in Expression of", gene_name) , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  ...
  #          colour = c("white","white"), 
  #          fill =c("darkorchid4","darkorchid4"))
f2+ scale_x_discrete(limits=c("Body Fat/Size",
            "Metabolism (inc. Diabetes)",
            "Tumours (Benign/Cancer)",
            "Atrophy/Dystrophy", 
            "Congenital Defects", 
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/f2COUNTvstraitgroup_REL_overview_SEATLE_NOOGEC.pdf"), width=200, height =100 , units="mm")
...
# INCOMPLETE FROM HERE ON, remove all steps on gene structure April22
...
write_csv(gene_and_reg, paste0(file_directory,"/",gene_name,"_test/Ensembl/",gene_name,"_and_reg_feat_FOR_GENE_DIAGRAM.csv"))
...
df_gtex_RED =df_gtex_RED%>% filter(!is.na(logp))
...
gene_structure_RED = ggplot()+
  geom_rect(gene_and_reg[which(gene_and_reg$EI !="Intron" & gene_and_reg$EI !="Exon"),], mapping=aes(xmin=Start, xmax=End, ymin=yminD, ymax=ymaxD,  fill=EI ),  alpha=0.8)+
  geom_rect(intron_hack, mapping=aes(xmin=gene_start, xmax=gene_end, ymin=yminD, ymax=ymaxD, fill = EI),  alpha=0.6)+
  geom_rect(gene_and_reg[which(gene_and_reg$EI =="Exon"),], mapping=aes(xmin=Start, xmax=End, ymin=yminD, ymax=ymaxD, color=EI))+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= paste0(gene_name, " Gene Region Diagram (Gtex after Merge SNPs)") , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Gene_diagram_output/Gene_structure_gtex_from_merge.pdf"))
```
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/RegRegion_COUNT_G_P.pdf"))
```
...
plot_grid(SNPcountperTissuegroup,SNP_by_traitgroupe0,pvalvstraitgroupe1,pvalvstraitgroupNO_O_e2_leg, nrow=4, ncol=1, align="v")
...
runApp( paste0(file_directory,"/summary_shiny_app.R"))
