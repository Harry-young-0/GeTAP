---
title: "Final_pipeline_wscraping"
author: "Harry Young"
date: "29/03/2022"
...
Finally, it is important to set up a file system similar to what I've done for it to run automatically. DRIVE:/YOUR FILE DIRECTORY/GENENAME_test/ containing Gtex and LDMatrix folders, each with Plots folder within. This will make the ctrl-f replace function work well. If you don't want to use your desktop you can ctrl-f and replace "DRIVE:/YOUR FILE DIRECTORY/" with whatever head folder you want: e.g. "~/OneDrive - University of Bristol/PhD/Epidemiology/" - 108 occurences should change

Here is an optional chunk to load in all the directories you'll need to run this notebook as is (as above simply change DRIVE:/YOUR FILE DIRECTORY/) to whatever your file path is to the master folder:


Any issues, first check the common errors section at the end of this document!

# Gnomad Data load in, MANUAL STEP
```{r Gnomad data load in using url}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
temp = list.files() # list of files within the directory
if(temp[1]!="gnomAD_v3.1.2_YOUR_GENE_NAME.csv"){
  browseURL("https://gnomad.broadinstitute.org/gene/YOUR_GENE_NAME?dataset=gnomad_r3")
  stop()
}
```
...
#####IMPORTANT BEFORE YOU START: Check the length (number of rows) of your unique_snp_list. I've set teh cutoff at 700, this may not work on less powerful computers
#### 1b - 2.1 Assess Linkage disequilibrium (D')
...
##### Get matrices of R2 and dprime
```{r Automated r2 and d prime START dataset}

if(length(unique_YOUR_GENE_NAME_Gtex_snps)<600){

setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/LDMatrix")
YOUR_GENE_NAME_r2 = LDmatrix(unique_YOUR_GENE_NAME_Gtex_snps, pop = "EUR", r2d = "r2", token ="generic_token")
...
dyplr(full_join) (also part of the tidyverse) works a treat for this kind of thing, but others work well too.
...
setwd("DRIVE:/YOUR FILE DIRECTORY/Trait and Tissue Groups")
...
## Step 1.1 Gnomad - START OF EDIT

### Data Load-in and initial clean
This time we need to go to gnomAD and download the appropriate dataframes
INCOMPLETE - needs testing since removal of gnomad2
```{r Initial load and clean gnomad}


setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
Gnomad3_YOUR_GENE_NAME = read_csv("gnomAD_v3.1.2_YOUR_GENE_NAME.csv")
Gnomad3_YOUR_GENE_NAME = Gnomad3_YOUR_GENE_NAME%>%
  rename(rsid = `rsIDs`)

df = Gnomad3_YOUR_GENE_NAME
head(df)  # This function just shows us the top few rows of the dataframe to see what we're looking at

#Also clear up any NA rsids that have snuck through
Gnomad3_YOUR_GENE_NAME = Gnomad3_YOUR_GENE_NAME[which(is.na(Gnomad3_YOUR_GENE_NAME$rsid)==F),]
...
Save this file as "cleaned" data:
```{r Save clean gnomad data}
# setwd repeated here 
# setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Quantitative and Qualitative SNP lists")
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(Gnomad3_YOUR_GENE_NAME, "YOUR_GENE_NAME_Gnomad3_cleaned.csv")

```

Then we need to isolate the unique SNPs for searching in the ieugwasR package:

```{r Unique SNPs from Gnomad}

unique_YOUR_GENE_NAME_Gnomad3_snps = unique(Gnomad3_YOUR_GENE_NAME$rsid)
# setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Quantitative and Qualitative SNP lists")
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")

write.table(unique_YOUR_GENE_NAME_Gnomad3_snps, "unique_YOUR_GENE_NAME_Gnomad3_snps.csv", sep="\t", row.names=FALSE, col.names=FALSE)
...
First step is to search your list of snps (unique_YOUR_GENE_NAME_Gnomad3_snps) into LDMatrix to get an interactive plot of linkage disequilibrium in your snp set. https://ldlink.nci.nih.gov/?tab=ldmatrix 
...
For gnomad as there are two dataframes (potentially 2.1 and 3.1) - no D' to simplify
### Step a - get matrices for r2

```{r Automated r2 and d prime START dataset gnomad3}

if(length(unique_YOUR_GENE_NAME_Gnomad3_snps)<600){

setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/LDMatrix")
YOUR_GENE_NAME_r2 = LDmatrix(unique_YOUR_GENE_NAME_Gnomad3_snps, pop = "EUR", r2d = "r2", token ="generic_token")
write_csv(YOUR_GENE_NAME_r2, "YOUR_GENE_NAME_gnomad3_r2.csv")



# open up the .csv file from the website as tab-deliminated 
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/LDMatrix")
YOUR_GENE_NAME_r2 = read_csv("YOUR_GENE_NAME_gnomad3_r2.csv")
head(YOUR_GENE_NAME_r2)
...
rm(df, YOUR_GENE_NAME_r2, Gtex_YOUR_GENE_NAME, linked_test0.5, linked_test0.75, linked0.5, linked0.75, temp, temp0.5D_2, temp0.75D_2, unique_snps, i,n,test)
##Save the environment
save.image("rs_overlap_ALL_YOUR_GENE_NAME_gnomad3.RData")
...
if(length(unique_YOUR_GENE_NAME_Gnomad3_snps)>=600){
  ...
  #INCOMPLETE 02/09
chunk <- 4000
n <- length(unique_YOUR_GENE_NAME_Gnomad3_snps)
r  <- rep(1:ceiling(n/chunk),each=chunk)[1:n]
d <- split(unique_YOUR_GENE_NAME_Gnomad3_snps,r)
library(LDlinkR)
 YOUR_GENE_NAME_LIMITED_SNP_LIST = SNPclip(
   d[[1]],
   pop = "EUR",
...
for(n in 2:length(d)){
 ...
 YOUR_GENE_NAME_LIMITED_SNP_LIST_temp = SNPclip(
   d[[n]],
   pop = "EUR",
   r2_threshold = "0.5",
   maf_threshold = "0.01",
  token = "generic_token" )
 YOUR_GENE_NAME_LIMITED_SNP_LIST = rbind(YOUR_GENE_NAME_LIMITED_SNP_LIST, YOUR_GENE_NAME_LIMITED_SNP_LIST_temp)
}
...
names(YOUR_GENE_NAME_LIMITED_SNP_LIST) = "rsid"
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/LDMatrix")
write_csv(YOUR_GENE_NAME_LIMITED_SNP_LIST, "YOUR_GENE_NAME_LIMITED_SNP_LIST_gnomad3.csv")
unique_YOUR_GENE_NAME_Gnomad3_snps = read_csv("YOUR_GENE_NAME_LIMITED_SNP_LIST_gnomad3.csv")# , col_names=F
unique_YOUR_GENE_NAME_Gnomad3_snps = unique_YOUR_GENE_NAME_Gnomad3_snps$rsid
}
...
Sometimes this works sometimes it doesn't try pressing play and if not:
RUN THIS CHUNK LINE BY LINE,  click the start of the line at pres ctrl-enter or command-enter for mac
for just gnomAD3.1.2 (see archive) :
```{r gnomad3 to phewas}
 Gnomad3_PHEWAS = phewas(unique_YOUR_GENE_NAME_Gnomad3_snps, pval = 1e-04) # Adjustable p-value here #

if(nrow(Gnomad3_PHEWAS)==0){
  print("Houston, we have a problem. Re run this chunk using the play button in the top right of the box. If that still doesn't work, run line by line using ctrl-enter OR cmd-enter. If it still doesn't work then I'm afraid something has gone really wrong! email me at hy14913@bristol.ac.uk")
} else{
# Then save using the following line:
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(Gnomad3_PHEWAS, "YOUR_GENE_NAME_GnomadtoPHEWAS_RAW.csv")
}
...
### For Gnomad 3a, initial merge and clean: 


Firstly, load in these libraries and start to reorganize the Gnomad-to-phewas RAW and original gnomAD data to get more of a feel for what the data looks like:
```{r initial read in of dataframes and ordering step gnomad}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
YOUR_GENE_NAME_GnomadtoPHEWAS_RAW = read_csv("YOUR_GENE_NAME_GnomadtoPHEWAS_RAW.csv") # load in Phewas output
YOUR_GENE_NAME_Gnomad_cleaned = read_csv("YOUR_GENE_NAME_Gnomad3_cleaned.csv") #  load in Gnomad input

YOUR_GENE_NAME_GnomadtoPHEWAS_RAW = YOUR_GENE_NAME_GnomadtoPHEWAS_RAW %>%
  rename(position_phewas = position, # Renaming columns to keep track of their original dataframe
         chr_phewas = chr)
 ...
 head(YOUR_GENE_NAME_Gnomad_cleaned)

 head(YOUR_GENE_NAME_GnomadtoPHEWAS_RAW)
```

Next step is to actually merge these two dataframes: Either of these work, I use merge because I found it first, but since I've been using dyplr this full_join is also nice. Worth checking that both produce the same dataframe as a test!

```{r Merging the gnomAD and phewas dataframes gnomAD}

test = merge(YOUR_GENE_NAME_Gnomad_cleaned, YOUR_GENE_NAME_GnomadtoPHEWAS_RAW, all=T, by = "rsid")
# OR
test2 = full_join(YOUR_GENE_NAME_Gnomad_cleaned, YOUR_GENE_NAME_GnomadtoPHEWAS_RAW, by = "rsid")
...
There'll be some rows where it only has gnomAD data, i.e. rsids that didn't come out with a result from PHEWAS searching.
To clean this, remove rows where there are NA's for multiple columns in PHEWAS section:

```{r cleaning up the merge step gnomAD}

to_remove = test[which(is.na(test$`HGVS Consequence`)==T),]
head(to_remove) # any rows with lots of NAs in Phewas columns

## Remove rows with NA in these PHEWAS columns
test = test %>% drop_na(trait) 
...
#setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Cross-Reference/GtextoPheWAS-Sorted/YOUR_GENE_NAME")
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(test, "YOUR_GENE_NAME_merge_Gnomad_PHEWAS.csv")
...
### 3a-2, removal of GTEx SNPs from gnomAD BEFORE LD removal.
```{r removal of GTEx SNPS from gnomAD to PheWAS dataset}
# reload in the original GTEx and gnomad Dataframes for safety 
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df_gnomad = read_csv("YOUR_GENE_NAME_merge_Gnomad_PHEWAS.csv")

setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex")
YOUR_GENE_NAME_Gtex_cleaned = read_csv("YOUR_GENE_NAME_Gtex_cleaned.csv")

# if the SNPs appear in the GTEx dataframe, remove them from the gnomAD dataframe:

gtex_snps =  unique(YOUR_GENE_NAME_Gtex_cleaned$rsid)
gtex_snps = as.data.frame(gtex_snps)
names(gtex_snps)= c("rsid")
gtex_snps$hidden_gtex_to_remove = "yes"

df_gnomad_new = full_join(df_gnomad, gtex_snps)

df_gnomad_new = df_gnomad_new[which(df_gnomad_new$hidden_gtex_to_remove != "yes"),]
df_gnomad_new = df_gnomad_new %>% dplyr::select(-c(hidden_gtex_to_remove))

```

```{r setting up a corrected beta column gnomad}
# Setup df as our merged dataframe:
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df = read_csv("YOUR_GENE_NAME_merge_Gnomad_PHEWAS.csv")
#Sometimes the alleles slip into TRUE/FALSE collumns so to correct that:
df$Reference = as.character(df$Reference)
df$Alternate= as.character(df$Alternate)
df$ea= as.character(df$ea)
df$nea = as.character(df$nea)


df$beta_cor = ifelse(df$ea == df$Alternate & df$nea == df$Reference, df$beta,
                     ifelse(df$ea == df$Reference & df$nea == df$Alternate, df$beta*-1, NA))

df %>% select (beta, beta_cor, ea, nea, Reference, Alternate)

if((sum(is.na(df$beta_cor)) == 0)==TRUE){
  print("There are no mutations that share an rsid that are completely different in Gnomad and PheWAS")
  
}else{paste0("There are traits to be removed due to different mutations with the same rsid, these are:", df[which(is.na(df$beta_cor)==T),][1])}
...
write_excel_csv(df, "YOUR_GENE_NAME_merge_Gnomad_PHEWAS-betacor.csv")
```
### 3b. ambiguous cases (AT  CG):
To solve reverse vs sense strand issues
```{r setting up an ambiguous case column gnomad}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df = read_csv("YOUR_GENE_NAME_merge_Gnomad_PHEWAS-betacor.csv")
...
#setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Cross-Reference/GnomadtoPheWAS-Sorted/YOUR_GENE_NAME")
write_excel_csv(df, "YOUR_GENE_NAME_merge_Gnomad_PHEWAS_betacor_ambig.csv")
...
### 3c-1. Trait grouping
I have gone through and categorised all the traits in the trait table from the open IEUOPENGWAS project. Sometimes there may be some minor case issues which you'll have to sort by hand (or as new studies are added to the database). I will make it available as both the original version that I've done and as a community modifiable version so we can keep it updated together!

We need to assign those groups to the main dataframe:

```{r Assign Trait Groups gnomad}
# read in our df (traits vs SNPs)
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df = read_csv( "YOUR_GENE_NAME_merge_Gnomad_PHEWAS_betacor_ambig.csv")
# read in trait database
setwd("/Users/hy14913/Local_work_folder/Epidemiology/Trait and Tissue Groups")
Full_trait_database = read_csv("Full_trait_database.csv")
...
# save unorganised
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(df, "YOUR_GENE_NAME_merge_Gnomad_PHEWAS_betacor_ambig_grouped.csv")
```
### 3c-2. Maybe mutation type sub categories here?

OK, looking good, we now have our new dataset, merged with the added columns for traitgroup.


### 3d. More Data Cleaning- removal of repeated traits INCOMPLETE, UPDATE WITH PREVIOUS NEW TRAITS
Firstly you need to clean up 'repeated traits'. This is annoying things like a comma in one name:
...
```{r Cleaning up duplicated traits gnomad}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df = read_csv("YOUR_GENE_NAME_merge_Gnomad_PHEWAS_betacor_ambig_grouped.csv")
...
### 4a. ukb-a is superfluous: - maybe too harsh in this dataset?
```{r Select and remove ukb-a, eval = FALSE}
df = df[grep("ukb-a", df$id, invert = TRUE), ]
...
#And finally save it:
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_csv(df, "YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL.csv")
```
...
```{r Removal of LD linked snps for r20.5 gnomaAD}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
YOUR_GENE_NAME_Gnomad_cleaned = read_csv("YOUR_GENE_NAME_Gnomad3_cleaned.csv")
unique_YOUR_GENE_NAME_Gnomad_snps= unique(YOUR_GENE_NAME_Gnomad_cleaned$rsid)
if(length(unique_YOUR_GENE_NAME_Gnomad_snps)<600){
rm(list=ls())
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df = read_csv("YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL.csv")
load("rs_overlap_ALL_YOUR_GENE_NAME_gnomad3.RData")  # specifically the dataframe with ambiguous data removed
## Annoyingly, in testing this stuff I found another source of error in my code. There are a whole load of snps that have been associated with a trait twice with similar p-values - I think these are OK because essentially they have been shown by two separate studies.
...
test2 = test2 %>% rename(beta_direction = beta_direction.y)
test2 = test2 %>% dplyr::select(-c("beta_direction.x"))
write_csv(test2, "YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL_linkagetempr20.5.csv")# full dataframe but with the "test" column 
...
write_csv(test, "YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv") # reduced df
} else{
  df = read_csv("YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL.csv")
  df$test ="filler"
  write_csv(df, "YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
}
...
## Step 5 Merging the Gnomad and Gtex dataframes DEC21
```{r Data load in step5 gnomad}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex")
df_gtex = read_csv("YOUR_GENE_NAME_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
YOUR_GENE_NAME_Gtex_cleaned = read_csv("YOUR_GENE_NAME_Gtex_cleaned.csv")
Full_tissue_database= read_csv("DRIVE:/YOUR FILE DIRECTORY/Trait and Tissue Groups/Full_tissue_database.csv")
YOUR_GENE_NAME_Gtex_cleaned =left_join(YOUR_GENE_NAME_Gtex_cleaned, Full_tissue_database) # assigning tissue groups
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
df_gnomad = read_csv( "YOUR_GENE_NAME_GnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")


```
To wean out SNPs that are actually GTEx, we can join the gnomAD final dataset (df_gnomad) with the original GTEx dataframe (YOUR_GENE_NAME_Gtex_cleaned). This means any SNPs with a value in the Tissue column is actually a GTEx SNP.

# INCOMPLETE, CHECK THIS STEP AFTER GTEx SNP removal moved to start. (3a-2)
```{r Merge of Original Gtex and df_gnomad}
YOUR_GENE_NAME_Gtex_cleaned_short = YOUR_GENE_NAME_Gtex_cleaned %>% dplyr::select(Tissue, tissue_group, tissue_group_secondary,rsid)
test =left_join(df_gnomad, YOUR_GENE_NAME_Gtex_cleaned_short, by="rsid")# Joining by rsid
test =test[which(is.na(test$Tissue)==T),] # find all rows with NA for tissue (non Gtex SNPS)
test2 = intersect(df_gnomad$rsid, YOUR_GENE_NAME_Gtex_cleaned$rsid) # GTEx SNPs within gnomAD dataframe.
test3 = intersect(test$rsid, df_gnomad$rsid) # overlap between reduced dataframe and original
test4 = unique(df_gnomad$rsid) # list of gnomad SNPS 
if(identical(setdiff(test4,test3), test2)==T){
  paste0( "All Gtex SNPs succesfully removed from df_gnomad, the unique gnomad snps are: ", test2)
}
df_gnomad_full =df_gnomad
df_gnomad=test
df_gnomad$`Gene Symbol` = "YOUR_GENE_NAME"
df_gnomad$origin = "gnomAD" 


# Logic: 1. if tissue = NA then it is a uniquely Gnomad SNP,
 # 2. the overlap of 

# Now we've reduced the df_gnomad frame to get rid of any Gtex SNPs, we can merge gnomad and Gtex final frames

# first step is getting rid of unused columns:
df_gnomad_shortened = subset(df_gnomad,select=-c(20:53))
# Join the dataframes
final_df = full_join(df_gtex,df_gnomad_shortened)
#Assign their origin frame
final_df$origin= ifelse(is.na(final_df$origin), "GTEx", "gnomAD")

# setting up a logp and directional logp column
final_df$p= ifelse(final_df$p==0, 1e-300,final_df$p)
...
write_csv(final_df, "/Users/hy14913/Local_work_folder/Epidemiology/YOUR_GENE_NAME_test/final_df.csv")
...
# INCOMPLETE: A KEY THING I HAVE MISSED IS ASSIGNING REG REGION AND VEP ANNOTATION - CAN BE DONE LATER!!!!


# Step 6: Plotting the data so far INCOMPLETE- NEEDS OVERHAUL(source code: Trait-tissue_comparer_MAIN_PLOTTING_SCRIPT.R):
...
(E2) pvalvstraitgroup_LOG no other
(E3) pvalvstraitgroup_LOG Other Gene Expression Change only
(E4) pvsTrait_INFL_LOG_TISCOL
(E5) pvsTrait_BODY_LOG_TISCOL
...
```{r Reading in the FINAL df and gnomad}
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
YOUR_GENE_NAME_Gnomad_cleaned = read_csv("YOUR_GENE_NAME_Gnomad3_cleaned.csv")
# df = read_csv("YOUR_GENE_NAME_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
df = read_csv("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/final_df.csv")
...
#### B - TissueGROUP-rsid_count_OVERVIEW
This plot shows the overview of our SNPs in each tissuegroup from th final df. (i.e after phewas search and LD removal but then filtered to remove duplicated tissuegroups from the trait matching process)
```{r}

Full_tissue_database = read_csv("DRIVE:/YOUR FILE DIRECTORY/Full_tissue_database.csv")
...
write_csv(LD_Gtex, "DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/_LDRED_Gtex.csv") # Storage of this dataframe for future use
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/TissueGROUP-rsid_count_OVERVIEW.pdf")


```


#### C - Correlation between RNA-seq/genotyped merged df - Checking the pattern isn't just beecause of this:
...
setwd("DRIVE:/YOUR FILE DIRECTORY/eGene") 
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/RNAvsMERGEDdfcount.pdf")
...
SNPcount = ggplot(LD_Gtex, aes(x=position_gtex,  fill = NES_direction)) + 
  ...
  labs(title= "Number of tissues each SNP Affects expression in" , x= paste0("Position (Chr ",Chromosome, ")"), y = "Number of Tissues")+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))  # setting up x axis labels so they don't overlap + no legend
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/Position_count_OVERVIEW.pdf")
```
...
pvsTrait = ggplot(temp, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/e1pvalvstraitgroup_LOG_all.pdf")
```
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/e2-pvalvstraitgroup_LOG_noother.pdf")
```

#####E3 only Other Gene Expression Change
The overview of all snps relative to traitgroups:
...
pvsTrait = ggplot(temp, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  # geom_jitter(aes(color=NES)) + # Jitter can be nicer but i think i prefer point with jitterdodge
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Genes Effected by SNPs that Change in Expression of YOUR_GENE_NAME" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
  #facet_grid(rows = vars(NES_direction))+ # facet splits the plot based on a variable, in this case NES direction
  #ylim(-200,200)+
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/e3-pvalvstraitgroup_LOG_ogec.pdf")
```

#### E4 Inflammation Traits:
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/e4-pvsTrait_INFL_LOG.pdf", width = 6, height =10)# Actually setting some dimensions due to long names
}
```

#### E5 Body fat/Size Traits only
```{r, }
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/e5pvsTrait_BODY_LOG_TISCOL.pdf")# Actually setting some dimensions due to long names
}

```

#### F1 Proportional count the snps in each beta direction:
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/f1COUNTvstraitgroup_REL_overview_SEATLE_all.pdf")
...
pvsTrait = ggplot()+ #Plot of Traitgroup vs SNP Count
  geom_bar(data= df[which(df$NES_direction=="Positive" & df$traitgroup!="Other Gene Expression Change"),], aes(x=traitgroup, fill=beta_direction, y=..count../nrow(df[which(df$NES_direction=="Positive"& df$traitgroup!="Other Gene Expression Change"),])),  position = position_dodge2(width = 1, preserve = "single"))+
  geom_bar(data= df[which(df$NES_direction=="Negative"& df$traitgroup!="Other Gene Expression Change"),], aes(x=traitgroup, fill=beta_direction, y=(..count../nrow(df[which(df$NES_direction=="Negative"& df$traitgroup!="Other Gene Expression Change"),]))*-1), position = position_dodge2(width = 1, preserve = "single"))+##  alpha is transparancy from 0-1
    geom_hline(yintercept=0, color ="gray70")+
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of YOUR_GENE_NAME" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  ...
  annotate("label",
           x=c(1,1),
           y=c(0.5,-0.5), # INCOMPLETE - reproducable version
           label=c("Positive","Negative"), 
           colour = c("white","white"), 
           fill =c("darkorchid4","darkorchid4"))
pvsTrait
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Final_plots/f2COUNTvstraitgroup_REL_overview_SEATLE_NOOGEC.pdf")


```

# Old plots, still, ran for people to look through if they'd like
...
Gtex_YOUR_GENE_NAME_START = read_csv("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Gtex_YOUR_GENE_NAME_START.csv")
...
ggsave("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/NESvsTiss_full.pdf")
```
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/NES_vs_tissue_and_overview")
...
Full_trait_database = read_csv("DRIVE:/YOUR FILE DIRECTORY/Trait and Tissue Groups/Full_trait_database.csv")
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots") #SAVE it
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/Tissue_Group_and_Trait_Group_overlap") #SAVE it
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/snp_against_Tissue") #SAVE it
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/p vs traitgroup split Expression direction") #SAVE it
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/p vs trait split by trait group and Expression direction") #SAVE it
...
#### New INFL with subcat - good plot, pretty useful if you have a lot of traits in a category
...
num_of_SNPs_by_beta = ggplot()+ #Plot of Traitgroup vs SNP Count
  geom_bar(data= df[which(df$NES_direction=="Positive"),], aes(x=trait, fill=beta_direction, y=..count../nrow(df[which(df$NES_direction=="Positive"),])), alpha=0.6, position = position_dodge2(width = 1, preserve = "single"))+
  geom_bar(data= df[which(df$NES_direction=="Negative"),], aes(x=trait, fill=beta_direction, y=(..count../nrow(df[which(df$NES_direction=="Negative" ),]))*-1), alpha=0.6, position = position_dodge2(width = 1, preserve = "single"))+##  alpha is transparancy from 0-1
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=1) + # I like this palette but you can remove it or change it 
...
## Firstly it is important to gather information about your gene structure, exon and intron locations.


##Attempted automation INCOMPLETE:
```{r}
...
'strand','exon_chrom_start','exon_chrom_end', '5_utr_start','5_utr_end','3_utr_start','3_utr_end', "hgnc_symbol")

gene_structure = getBM(attributes=c(getAtt,"ensembl_transcript_id"),filters=c("hgnc_symbol","with_ccds"),value=list("YOUR_GENE_NAME",TRUE),mart=ensembl)

write_csv(gene_structure,"DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Ensembl/YOUR_GENE_NAME_structure_FIN.csv")

# now I need to learn how to convert these into usable bits
```
## Assigning Exon, intron and UTR to the final_df
```{r}
...
### Preamble, data setup steps - could I automate this somehow?! maybe using biomart
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Ensembl/All_Exon_Options")
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Ensembl")
...
# setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex")
# df = read_csv("final_df") # from before
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Ensembl") # saving in the Ensembl folder
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gene_diagram_output")
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gtex/Plots/REG_region_counts") #SAVE it
...
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Circos")
...
# New section - comparing Gnomad and Gtex directly: 
Data Preamble
```{r}


setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Trait and Tissue Groups")
Full_tissue_database = read_csv("Full_tissue_database.csv")
Full_trait_database = read_csv("Full_trait_database.csv")

setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
YOUR_GENE_NAME_Gnomad2_cleaned = read_csv("YOUR_GENE_NAME_Gnomad2_cleaned.csv")
YOUR_GENE_NAME_Gnomad3_cleaned = read_csv("YOUR_GENE_NAME_Gnomad3_cleaned.csv")
YOUR_GENE_NAME_Gnomad_cleaned = full_join(YOUR_GENE_NAME_Gnomad2_cleaned, YOUR_GENE_NAME_Gnomad3_cleaned)
# df = read_csv("YOUR_GENE_NAME_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
df_gnomad = read_csv("YOUR_GENE_NAME_gnomadPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
df_gnomad = as_data_frame(df_gnomad)
df_gnomad$traitgroup =as.character(df_gnomad$traitgroup)
is.character(df_gnomad$traitgroup)
...
df_gnomad$traitgroup = df_gnomad$traitgroup[order(match(df_gnomad$traitgroup,y))]
# No idea why this stopped working?!
# df_gnomad$traitgroup = factor(df$traitgroup, levels =c("Body Fat/Size",
#             "Metabolism (inc. Diabetes)",
#              "Tumours (Benign/Cancer)",
#             "Atrophy/Dystrophy", 
#             "Congenital Defects", 
...
df_gtex$traitgroup = df_gtex$traitgroup[order(match(df_gtex$traitgroup,y))]
df_gtex =subset(df_gtex, select=-c(traitgroup))
df_gtex = left_join(df_gtex, Full_trait_database)


```


## What are the trait group consequences of the gnomAD SNPS compared to the GTEx SNPS:
```{r}


df_gtex$beta_direction = factor(df_gtex$beta_direction, levels=c("Positive", "Negative"))
df_gnomad$beta_direction = factor(df_gnomad$beta_direction, levels=c("Positive", "Negative"))

df_gtex_short=df_gtex %>% select(traitgroup,Subcat,Alt_Subcat,logp,beta_direction, rsid)
df_gnomad_short = df_gnomad %>% select(traitgroup,Subcat,Alt_Subcat,logp,beta_direction, rsid)
df_gtex_short$ID = "GTEx"
df_gnomad_short$ID = "gnomAD"

df = rbind(df_gtex_short,df_gnomad_short)
df$traitgroup= ifelse(df$Subcat=="Other Gene Expression Change", df$Subcat, df$traitgroup)
...
# First plot of all the SNPS from gtex or Gnomad, regardless of LD , overlap etc etc

ggplot(data =df, aes(x=traitgroup, y=logp, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  # geom_jitter(aes(color=NES)) + # Jitter can be nicer but i think i prefer point with jitterdodge
  geom_point( alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Trait Groups Effected by gnomAD or GTEx SNPs of YOUR_GENE_NAME" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
  facet_grid(cols = vars(ID), space="free")+
  theme(axis.text.x = element_text(angle = 45, hjust =1))
...
# 
# setwd("/Users/hy14913/Local_work_folder/Epidemiology/Comparisons_between_gtexvgnomad") # save in this dedicated folder
# ggsave("gtexvgnomad_logp.pdf")


# Second plot of all the SNPS , removing all SNPS that appear in both sets:
df = rbind(df_gtex_short[1:6], df_gnomad_short[1:6])
df = unique(df)
df$ID = "Both"
df = rbind(df,df_gtex_short, df_gnomad_short)

ggplot(data =df, aes(x=traitgroup, y=logp, color=beta_direction)) + 
  geom_point( alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Trait Groups Effected by gnomAD or GTEx SNPs of YOUR_GENE_NAME" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
...
# setwd("/Users/hy14913/Local_work_folder/Epidemiology/Comparisons_between_gtexvgnomad") # save in this dedicated folder
# ggsave("gtexvgnomadvboth_logp.pdf")
```

# Gtex SNPs hidden in Gnomad
The next issue we face is whether a lot of the Gnomad SNPs are also GTEx SNPs.
```{r}
# check whether any of the SNP ID's match between the final gnomad dataframe and our original GTEx dataframe.

YOUR_GENE_NAME_gtex_originals =YOUR_GENE_NAME_gtex_cleaned[9:10] #sleecting rsid and one other collumn for simplicity
names(YOUR_GENE_NAME_gtex_originals) = c("rsid", "Database")
YOUR_GENE_NAME_gtex_originals[2] ="GTEx" 
YOUR_GENE_NAME_gtex_originals = unique(YOUR_GENE_NAME_gtex_originals) # getting rid of duplicates 

df_gnomad_short_gtex_merge = left_join(df_gnomad_short, YOUR_GENE_NAME_gtex_originals)

df_gnomad_short_gtex_merge$Database =ifelse(is.na(df_gnomad_short_gtex_merge$Database)==T, "gnomAD", df_gnomad_short_gtex_merge$Database)
nrow(df_gnomad_short_gtex_merge[which(df_gnomad_short_gtex_merge$Database=="GTEx"),]) # 309/666 are Gtex

df_gnomad_short_gtex_merge$traitgroup = df_gnomad_short_gtex_merge$traitgroup[order(match(df_gnomad_short_gtex_merge$traitgroup,y))]

ggplot(data =df_gnomad_short_gtex_merge, aes(x=traitgroup, y=logp, color=beta_direction)) + 
  geom_point( alpha=0.6, position = position_jitterdodge())+ 
  scale_colour_viridis_d(direction=-1) +
  labs(title= "Trait Groups Effected by gnomAD SNPs of YOUR_GENE_NAME" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
  facet_grid(cols = vars(Database), space="free")+
  theme(axis.text.x = element_text(angle = 45, hjust =1))
# setwd("/Users/hy14913/Local_work_folder/Epidemiology/Comparisons_between_gtexvgnomad") # save in this dedicated folder
# ggsave("gtex_hidden_in_gnomad_logp.pdf")

```

Or alternatively we can view this nicely as a bar chart showing the proportion of SNPs that are GTEx SNPs or purely gnomAD in each trait group within the final gnomAD dataframe:
```{r}
# stacked bar chart
ggplot(data =df_gnomad_short_gtex_merge, aes(x=traitgroup, fill=Database)) + 
  geom_bar()+ 
  scale_fill_viridis_d(direction=-1) +
  labs(title= "Trait Groups Effected by gnomAD SNPs of YOUR_GENE_NAME" , x= "Trait Group", y= "Number of SNPs")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
  #facet_grid(cols = vars(Database), space="free")+
  theme(axis.text.x = element_text(angle = 45, hjust =1))

# setwd("/Users/hy14913/Local_work_folder/Epidemiology/Comparisons_between_gtexvgnomad") # save in this dedicated folder
# ggsave("gtex_hidden_in_gnomad_bar.pdf")

```
JAN22
# INCOMPLETE - needs testing, may need simplifying to just gnomAD3 (see extra chunk)
```{r,Phewas searching must run line by line using ctrl-enter}
# Load in package - should have been done at start but here too
library(ieugwasr)

# change out list of snps from unique_YOUR_GENE_NAME_Gnomad2_snps to LIMITED_SNP_LIST here depending on choice above
 Gnomad2_PHEWAS = phewas(unique_YOUR_GENE_NAME_Gnomad2_snps, pval = 1e-04) # Adjustable p-value here #

if(nrow(Gnomad2_PHEWAS)==0){
  print("Houston, we have a problem. Re run this chunk using the play button in the top right of the box. If that still doesn't work, run line by line using ctrl-enter OR cmd-enter. If it still doesn't work then I'm afraid something has gone really wrong! email me at hy14913@bristol.ac.uk")
} else{
# Then save using the following line:
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(Gnomad2_PHEWAS, "YOUR_GENE_NAME_Gnomad2toPHEWAS_RAW.csv")
}
...
# Then save using the following line:
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(Gnomad3_PHEWAS, "YOUR_GENE_NAME_Gnomad3toPHEWAS_RAW.csv")
}

if(length(intersect(Gnomad2_PHEWAS$rsid,Gnomad3_PHEWAS$rsid)) == length(unique(Gnomad2_PHEWAS$rsid)) &
length(intersect(Gnomad2_PHEWAS$rsid,Gnomad3_PHEWAS$rsid)) == length(unique(Gnomad3_PHEWAS$rsid))&
nrow(Gnomad2_PHEWAS)==nrow(Gnomad3_PHEWAS)){
  print("The Snps from gnomad2 and 3 are the same so we will save one file of YOUR_GENE_NAME_GnomadtoPHEWAS_RAW.csv for simplicity")
  setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(Gnomad3_PHEWAS, "YOUR_GENE_NAME_GnomadtoPHEWAS_RAW.csv")
  
} else {
  print("There are different traits in the two dataframes therefore we need to bind them together")
  Gnomad_comb_PHEWAS = full_join(Gnomad2_PHEWAS, Gnomad3_PHEWAS)
  setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
  write_excel_csv(Gnomad_comb_PHEWAS, "YOUR_GENE_NAME_GnomadtoPHEWAS_RAW.csv")
}
```

INCOMPLETE - remove gnomad2 stuff?
```{r Initial load and clean gnomad}

setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
Gnomad2_YOUR_GENE_NAME = read_csv("gnomAD_v2.1.1_YOUR_GENE_NAME.csv")
Gnomad3_YOUR_GENE_NAME = read_csv("gnomAD_v3.1.2_YOUR_GENE_NAME.csv")
Gnomad2_YOUR_GENE_NAME = Gnomad2_YOUR_GENE_NAME%>%
  rename(rsid = `rsIDs`)
Gnomad3_YOUR_GENE_NAME = Gnomad3_YOUR_GENE_NAME%>%
  rename(rsid = `rsIDs`)

df = Gnomad2_YOUR_GENE_NAME
head(df)  # This function just shows us the top few rows of the dataframe to see what we're looking at

#Also clear up any NA rsids that have snuck through
Gnomad2_YOUR_GENE_NAME = Gnomad2_YOUR_GENE_NAME[which(is.na(Gnomad2_YOUR_GENE_NAME$rsid)==F),]
Gnomad3_YOUR_GENE_NAME = Gnomad3_YOUR_GENE_NAME[which(is.na(Gnomad3_YOUR_GENE_NAME$rsid)==F),]
...
# setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Quantitative and Qualitative SNP lists")
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write_excel_csv(Gnomad2_YOUR_GENE_NAME, "YOUR_GENE_NAME_Gnomad2_cleaned.csv")
write_excel_csv(Gnomad3_YOUR_GENE_NAME, "YOUR_GENE_NAME_Gnomad3_cleaned.csv")
...
```{r Unique SNPs from Gnomad}
unique_YOUR_GENE_NAME_Gnomad2_snps = unique(Gnomad2_YOUR_GENE_NAME$rsid)
unique_YOUR_GENE_NAME_Gnomad3_snps = unique(Gnomad3_YOUR_GENE_NAME$rsid)
# setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Quantitative and Qualitative SNP lists")
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/Gnomad")
write.table(unique_YOUR_GENE_NAME_Gnomad2_snps, "unique_YOUR_GENE_NAME_Gnomad2_snps.csv", sep="\t", row.names=FALSE, col.names=FALSE) # saving the file
write.table(unique_YOUR_GENE_NAME_Gnomad3_snps, "unique_YOUR_GENE_NAME_Gnomad3_snps.csv", sep="\t", row.names=FALSE, col.names=FALSE)


length(intersect(unique_YOUR_GENE_NAME_Gnomad2_snps,unique_YOUR_GENE_NAME_Gnomad3_snps))
...
##2.1 Assess Linkage disequilibrium (D')
First step is to search your list of snps (unique_YOUR_GENE_NAME_Gnomad2_snps) into LDMatrix to get an interactive plot of linkage disequilibrium in your snp set. https://ldlink.nci.nih.gov/?tab=ldmatrix 
...
For gnomad as there are two dataframes (potentially 2.1 and 3.1) - no D' to simplify
### Step a - get matrices for r2
```{r Automated r2 and d prime START dataset gnomad2}

if(length(unique_YOUR_GENE_NAME_Gnomad2_snps)<600){

setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/LDMatrix")
YOUR_GENE_NAME_r2 = LDmatrix(unique_YOUR_GENE_NAME_Gnomad2_snps, pop = "EUR", r2d = "r2", token ="generic_token")
write_csv(YOUR_GENE_NAME_r2, "YOUR_GENE_NAME_gnomad2_r2.csv")



# open up the .csv file from the website as tab-deliminated 
setwd("DRIVE:/YOUR FILE DIRECTORY/YOUR_GENE_NAME_test/LDMatrix")
YOUR_GENE_NAME_r2 = read_csv("YOUR_GENE_NAME_gnomad2_r2.csv")
head(YOUR_GENE_NAME_r2)
...
rm(df, YOUR_GENE_NAME_r2, Gtex_YOUR_GENE_NAME, linked_test0.5, linked_test0.75, linked0.5, linked0.75, temp, temp0.5D_2, temp0.75D_2, unique_snps, i,n,test)
##Save the environment
save.image("rs_overlap_ALL_YOUR_GENE_NAME_gnomad2.RData")
...
# WARNING THIS STEP WILL NOT WORK WITH >11590 SNPs
if(length(unique_YOUR_GENE_NAME_Gnomad2_snps)>=600){
chunk <- 4000
n <- length(unique_YOUR_GENE_NAME_Gnomad2_snps)
r  <- rep(1:ceiling(n/chunk),each=chunk)[1:n]
d <- split(unique_YOUR_GENE_NAME_Gnomad2_snps,r)
library(LDlinkR)
 YOUR_GENE_NAME_LIMITED_SNP_LIST = SNPclip(
   d[[1]],
   pop = "EUR",
...
write_csv(YOUR_GENE_NAME_LIMITED_SNP_LIST, "YOUR_GENE_NAME_LIMITED_SNP_LIST_gnomad2.csv")
unique_YOUR_GENE_NAME_Gnomad2_snps = read_csv("YOUR_GENE_NAME_LIMITED_SNP_LIST_gnomad2.csv")# , col_names=F
unique_YOUR_GENE_NAME_Gnomad2_snps = unique_YOUR_GENE_NAME_Gnomad2_snps$rsid
}
```
