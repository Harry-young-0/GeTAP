...
### Please email me at harry.young@manchester.ac.uk with any questions about the pipeline
Please see instructions on github / in readme.md for details
# Assign your gene name, file directory, LD token and ieu token
...
# -------------------------- WARNING ----------------------------
## GTEx v10 has introduced some gene issues: Please use this overide wherever possible, it is safer! 
## Got to https://gtexportal.org/home/ search your gene and get the gencode ID from the single tissue header:
## e.g: Significant Single-Tissue eQTLs for NFE2L2 (ENSG00000116044.16) in all tissues

GENCODEID = "YOUR_GENCODEID" #  e.g: "NFE2L2" = "ENSG00000116044.16"
#e.g gene_name ="NFE2L2" ; "YOUR_GENE_NAME" #ABI2 #ACAD8 #MBP #ROM1
...
#windows e.g: file_directory = "C:/Users/Harry/Documents/Local_work_folder"
#mac e.g: file_directory ="/Users/Harry/Documents/Local_work_folder/JUL_TEST"

#3. LD link token setup
LD_token = "abc1bc2bc3bc"
# 12 character token e.g:"abc1bc2bc3bc" - generic_token


#4. ieu_openGWAS token:
ieu_token="YOUR_IEU_TOKEN_HERE"
# ieu_token = "YOUR_IEU_TOKEN_HERE"

# assign a value of 1 or 2, 1 = faster but more likely to error; 2 = slower but less likely to error (DEFAULT= faster) INCOMPLETE - FEATURE TO ADD
speed_vs_reliability = 1
...
### List of things to add with time
sQTL
gnomad
Progress through batches
how fast to run (set up a n=3 for how many snps per thing, and make sure that it's not being removed in the LD step)
...
if(file.exists(paste0(file_directory, "/Trait and Tissue Groups"))){
  print("database folder exists - moving on")
}else{
...
if(file.exists(paste0(file_directory,"/" ,gene_name,"_test"))){
   print("file exists - moving on")
    ...
    print("no previous folder - creating a new one")
...
# Test for previous run through GTEx
```{r}
Pre_phewas_test=file.exists(paste0(file_directory,"/", gene_name,"_test/Final_data/Pre_PheWAS_save.RData"))
if(Pre_phewas_test==T){
  load(paste0(file_directory,"/", gene_name,"_test/Final_data/Pre_PheWAS_save.RData"))
  Pre_phewas_test=file.exists(paste0(file_directory,"/",gene_name,"_test/Final_data/Pre_PheWAS_save.RData"))
}
```
# ieuopenGWAS setup:
As of May 2024, openGWAS requires an authentification token to help with server traffic, this is a little it more involved for your non-R user... hopefully I can do it automatically:

A user will need to generate their own token and copy it into the first chunk I think
```{r}
## To use the IEU OpenGWAS, you need to authenticate your account

## To authenticate, you need to generate a token from the OpenGWAS website using the following steps:

## Login to https://api.opengwas.io/profile/

## Generate a new token

## Open your .Renviron file by typing "file.edit("~/.Renviron")" into the console and adding "OPENGWAS_JWT=<token>" (i.e., replacing the "<token>" with your own token") and saving

## Restart your R session

## To check that your token is being recognised, run the following code:

##ieugwasr::get_opengwas_jwt()

##ieugwasr::user(opengwas_jwt = get_opengwas_jwt())

## If the first command returns a long random string then, you are authenticated.

## The second command will make a request to the API for your user information using your token. 

## It should return a list with your user information. If it returns an error, then your token is not working
```
...
This is done automatically using their v2 API entry to their database.
### Updated GTEx Query using gtexr package

```{r, new GTEX start}
if(Pre_phewas_test==F){
  print("previous run not detected")
  if(exists("Gtex_1")==T){
    print("df exists so moving on")
  }else{
if(exists("GENCODEID")){
	if(GENCODEID =="YOUR_GENCODEID"){
stop("Generic GENCODEID detected, please enter GENCODEID from GTEx: e.g "NFE2L2" would be "ENSG00000116044.16"
	}
  Gtex_1 = get_significant_single_tissue_eqtls(
  gencodeIds = GENCODEID,
  # variantIds = NULL,
  ...
  GENCODEID = get_genes( # NEED OVERRIDE HERE!!
    gene_name,
  ...
  if(nrow(GENCODEID)==0){
    stop("Failed to find SNPs, this is most often a gencodeID issue. Please use override in first chunk!")
  }
  GENCODEID=GENCODEID$gencodeId
  ...
  df = df %>%
    separate(`Variant Id`, c("chr_gtex", "position_gtex", "a", "b", "build_gtex"), remove = F) #remove = F keeps the original column too
...
if(Pre_phewas_test==F){
  ...
  write_csv(as.data.frame(unique_Gtex_snps), paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps_pre_r2.csv")) # saving the file
  write_csv(as.data.frame(unique_Gtex_snps), paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps.csv"))
}
```
...
if(Pre_phewas_test==F){
  ...
  if(length(unique_Gtex_snps)<400){
  ...
  rm(linked_test0.5, linked_test0.75, linked0.5, linked0.75, n,test,Gtex_1,r2)
 ...
 # ALternate version if too many SNPs to run through PheWAS 
    if(length(unique_Gtex_snps)>=400){
    ...
    names(LIMITED_SNP_LIST) = "unique_Gtex_snps"
    
    write_csv(LIMITED_SNP_LIST, paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps.csv")) # if this happens it'll overwrite the unique snps frame
    }
  }
  
  save.image(paste0(file_directory,"/", gene_name,"_test/Final_data/Pre_PheWAS_save.RData"))
}

# Also need to add a check in here that allows for a callback if the phewas search returned is too big! INCOMPLETE April 2024
```
...
```{r,Phewas searching }
# New Phewas searching attempt:

batch_size=1

#Setup for batching if slow
batch_for_phewas = c("ieu-b",
                     #"ubm-b",
                     "ebi-a",
                     "eqtl-a",
                     "ukb-b",
...
unique_Gtex_snps_df_storage = read_csv(paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps.csv"))
unique_Gtex_snps = unique_Gtex_snps_df_storage$unique_Gtex_snps
unique_Gtex_snps2=unique_Gtex_snps # storage of whole list for final check
number_of_runs_needed = length(na.omit(unique_Gtex_snps2))%/%batch_size + 1 # set number of runs based on total unique snps / batch size
unique_Gtex_snps =na.omit(unique_Gtex_snps)

if(!exists("Gtex_PHEWAS")){
column_names <- c("beta", "se", "chr", "p", "n", "position", "id", "rsid", "ea", "nea", "eaf", "trait")
Gtex_PHEWAS <- data.frame(matrix(ncol = length(column_names), nrow = 0))
colnames(Gtex_PHEWAS) <- column_names
}
if(!exists("SNPs_run")){
  SNPs_run =c()
}
# This loop is for batch size of 1#####
if(batch_size==1){
  number_of_runs_needed = length(unique_Gtex_snps)
  # Have we tried this already? 
  ##If so lets pick up where we left of:
  if(exists("Gtex_PHEWAS_list")){
    progress = length(SNPs_run) +1
    for(n in progress:number_of_runs_needed){
      test = unique_Gtex_snps[n] # select batch of SNPS
      print(paste0("Temporary dataframe ",n)) # print progress for visual satisfaction
      Gtex_PHEWAS_2 = phewas(test, pval = 1e-04, opengwas_jwt = ieu_token) # search batch of SNPs in phewas
      did_phewas_error = length(Gtex_PHEWAS_2)==10
      if(did_phewas_error==T){
        break
      }else{
          SNPs_run=c(SNPs_run,test)
        }
      did_phewas_work = length(Gtex_PHEWAS_2)==12 # test to see if phewas output is working:###

      if(did_phewas_work==T){
        paste0("PheWAS search for ", unique_Gtex_snps[n], " worked!")
        assign(value=Gtex_PHEWAS_2,x=paste0("Gtex_PHEWAS_run_",unique_Gtex_snps[n])) # new dataframe _n
        Gtex_PHEWAS_list =mget(ls(pattern = "Gtex_PHEWAS_run_"), envir = .GlobalEnv)
        Sys.sleep(10)
        }
    
      }
    Gtex_PHEWAS_temp = bind_rows(Gtex_PHEWAS_list)
    Gtex_PHEWAS = rbind(Gtex_PHEWAS,Gtex_PHEWAS_temp)
    Gtex_PHEWAS =unique(Gtex_PHEWAS)
  }else{
  
  ##If not, lets start from the beginning:
  ###Select unique SNPs, split into batches and run through Phewas
  # Set up a loop for each run n=1, n=2 and test = snps 1:3 , 4:6 etc if batch_zie =3
  for(n in 1:number_of_runs_needed){
    test = unique_Gtex_snps[n] # select batch of SNPS
    print(paste0("Temporary dataframe ",n)) # print progress for visual satisfaction
    Gtex_PHEWAS_2 = phewas(test, pval = 1e-04, opengwas_jwt = ieu_token) # search batch of SNPs in phewas
    did_phewas_error = length(Gtex_PHEWAS_2)==10
...
# INCOMPLETE JULY 24 - NEED TO ADD DID PHEWAS WORK CHECK TO ALL
# This loop doesn't bother splitting each phewas query by dataset - just searches the SNP####

# if(batch_size>1){ # skip these two options if batch size is 1
#   if(speed_vs_reliability==1){
#     # Have we tried this already? 
#     ##If so lets pick up where we left of:
#     if(exists("Gtex_PHEWAS_list")){
#       progress = length(Gtex_PHEWAS_list) +1
#       for(n in progress:number_of_runs_needed){
#         test = na.omit(unique_Gtex_snps[(((n-1)*batch_size)+1):(n*batch_size)]) # select batch of SNPS
#         print(paste0("Temporary dataframe ",n)) # print progress for visual satisfaction
#         Gtex_PHEWAS_2 = phewas(test, pval = 1e-04, opengwas_jwt = ieu_token) # search batch of SNPs in phewas
#         assign(value=Gtex_PHEWAS_2,x=paste0("Gtex_PHEWAS_run_",n)) # new dataframe _n
#         Gtex_PHEWAS_list =mget(ls(pattern = "Gtex_PHEWAS_run_"), envir = .GlobalEnv)
#         Sys.sleep(10)
#       }
#       
...
#       test = na.omit(unique_Gtex_snps[(((n-1)*batch_size)+1):(n*batch_size)]) # select batch of SNPS
#       print(paste0("Temporary dataframe ",n)) # print progress for visual satisfaction
#       Gtex_PHEWAS_2 = phewas(test, pval = 1e-04) # search batch of SNPs in phewas
#       assign(value=Gtex_PHEWAS_2,x=paste0("Gtex_PHEWAS_run_",n)) # new dataframe _n
#       Gtex_PHEWAS_list =mget(ls(pattern = "Gtex_PHEWAS_run_"), envir = .GlobalEnv)
#     }
# 
...
#   }
#   
#   #This Loop splits into batches based on phewas datasets####
#   if(speed_vs_reliability==2){
#     # Have we tried this already? 
#     ##If so lets pick up where we left of:
#     if(exists("Gtex_PHEWAS_list")){
#       progress = length(Gtex_PHEWAS_list) +1
...
#         test = na.omit(unique_Gtex_snps[(((n-1)*batch_size)+1):(n*batch_size)]) # select batch of SNPS
#         print(paste0("Temporary dataframe ",n)) # print progress for visual satisfaction
#         # Split per batch of phewas
#         for(id in 1:length(batch_for_phewas)){
#           print(batch_for_phewas[id])
#           # Ideally I need some sort of progress saving step here! - only relevent if I have completed SNPS anyway?
#           # 
#           Gtex_PHEWAS_temp = phewas(test,
#                                     batch = batch_for_phewas[id],
#                                     pval = 1e-04,
#                                     opengwas_jwt = ieu_token)
#           if(nrow(Gtex_PHEWAS_temp>0)){
#             assign(value=Gtex_PHEWAS_temp,x=paste0("Gtex_PHEWAS_temp_",id))
#           }
#           Sys.sleep(10)
#           Gtex_PHEWAS_2 =mget(ls(pattern = "Gtex_PHEWAS_temp_"), envir = .GlobalEnv)
#           Gtex_PHEWAS_2 = bind_rows(Gtex_PHEWAS_2)
#         }
#       
#         assign(value=Gtex_PHEWAS_2,x=paste0("Gtex_PHEWAS_run_",n)) # new dataframe _n
#         Gtex_PHEWAS_list =mget(ls(pattern = "Gtex_PHEWAS_run_"), envir = .GlobalEnv)
...
if(nrow(Gtex_PHEWAS)==0){
  ERROR_PheWAS ="Houston, we have a problem. Re run this chunk using the play button in the top right of the box. If that still doesn't work, run line by line using ctrl-enter OR cmd-enter. If it still doesn't work then I'm afraid something has gone really wrong! email me at harry.young@bristol.ac.uk"
} else{
  write_excel_csv(Gtex_PHEWAS, paste0(file_directory,"/", gene_name,"_test/Final_data/",gene_name,"_GtextoPHEWAS_RAW.csv"))
}

progress = length(SNPs_run)
if(progress==number_of_runs_needed){
...
# line to save progress to check!
objects_to_remove <- ls(pattern = "Gtex_PHEWAS_run_")
rm(list = objects_to_remove, envir = .GlobalEnv)

objects_to_remove <- ls(pattern = "Gtex_PHEWAS_temp")
rm(list = objects_to_remove, envir = .GlobalEnv)

last_run_batch_size = batch_size
```
...
Merge these two dataframes: dyplr full_join or merge both work.
...
df$a =if_else(df$a==T, "T",df$a)

df$b =if_else(df$b==T, "T",df$b)

df$ea =if_else(df$ea==T, "T",df$ea)

df$nea =if_else(df$nea==T, "T",df$nea)
...
```
For the moment we are just labeling the ambiguous data, we can filter it out when plotting.
...
### 3c-1. Trait grouping
I have categorised all the traits in the trait table from the open IEUOPENGWAS project (use ieugwasr::gwasinfo() to get the original data. 

You can alter the downloaded Full-Trait_database.csv file to incorporate your own groupings.
...
if(sum(is.na(df$traitgroup))==0){
  ...
  paste(unique(df$trait[which(is.na(df$traitgroup)==T)]), "needs to be added to Full_trait_database") # VERY UNLIKELY!
}
...
# Need a check to make sure that df isn't so big it break everything
#Data preamble##
unique_Gtex_snps= read_tsv(paste0(file_directory,"/", gene_name,"_test/Gtex/unique_Gtex_snps_pre_r2.csv"), col_names = FALSE) # read the original unique snps file
...
#Storage line for remerge if needed:
#df = df %>% dplyr::select(-traitgroup,Subcat,Alt_Subcat)%>% left_join(., Full_trait_database)
df = unique(df)
...
grid_col = c(`Body Fat/Size` = "#0DF485",
            ...
            `Congenital Defects` = "#3277DC",
             `Cardiovascular` = "#4532DC",
             `Blood/Vascular` ="#6C39FF",
             Inflammation ="#9A32DC", #  or #AA98A970
            `Respiratory` ="#C402DC",
             `Renal/Urinary` = "#DC32D0",
            `Physical Activity` = "#DC024D",
             `Digestive` ="#DC2532",
            `Microbiome` = "#DC3D10",
            `Nervous System/Brain` = "#DC5D32",
             `Bone (inc. Dental)` = "#DC8732",
            `Muscle/Tendon` = "#DCA732",
            `Physical Injury` = "#DCD232",
            `Alcohol-related Disease` ="#BCDC32",
             `Life Span` ="#92DC32",
             `Female Specific` = "#43DC32",
            `Male Specific` = "#00f556",
...
A1SNPcountperTissuegroup = ggplot(test, aes(x=tissue_group,y=count, fill=NES_direction)) + 
  ...
  labs(title= paste0("Number of SNPs per Tissue from GTEx\n(before PheWAS and LD removal)") , x= "Tissue Group", y=paste("Number of SNPs"))+ #labels
...
#### B - Correlation between nmuber of SNPs in each trait group compared to trait database category size - Checking the pattern isn't just beecause of this:
Logic behind this correlation that if this is considered a completely random distribution of traits, what pattern would you expect to see. therefore if you take the number of STAs per traitgroup compared to the total number of STAs and correlate it to the number of traits in the same traitgroup relative to the entire database.

Issue is that OGEC throws this metric way off - removed for the sake of argument.

```{r Plotting the correlation between traits and trait databse}
Full_trait_database = read_csv(paste0(file_directory,"/Trait and Tissue Groups/Full_trait_database.csv"))
Full_trait_database = unique(Full_trait_database)
df_RED = df %>% 
  dplyr::select(trait, traitgroup, p, rsid) 
df_RED = unique(df_RED)

df_forcomp = df_RED %>%
  group_by(traitgroup) %>% 
  summarise(count = length(traitgroup[!is.na(traitgroup)])) %>% 
  dplyr::select(traitgroup, count)

TG_count = Full_trait_database %>% 
  group_by(traitgroup) %>% 
  summarise(count = length(traitgroup[!is.na(traitgroup)]))
...
comparison_TG =comparison_TG %>% filter(traitgroup!="Other Gene Expression Change")
...
comparison_TG$traitgroup = factor(comparison_TG$traitgroup, levels = trait_order)
...
B2 = BCorrelation_tg +xlim(c(0,20))+ylim(c(0,20))
B2
...
INCOMPLETE: THIS NEEDS A MOD, CURRENTLY GIVING RELATIVE TO JUST THE NEGS OR POSS, NEEDS TO BE POS / ALL and NEg/ ALL
...
E1 = ggplot()+ #Plot of Traitgroup vs SNP Count
  geom_bar(data= temp_pos, aes(x=factor(traitgroup), fill=beta_direction, y=after_stat(count)/nrow(temp_full)),  position = position_dodge2(width = 1, preserve = "single"))+
  geom_bar(data= temp_neg, aes(x=factor(traitgroup), fill=beta_direction, y=(after_stat(count)/nrow(temp_full))*-1), position = position_dodge2(width = 1, preserve = "single"))+##  alpha is transparancy from 0-1
...
temp_neg = df %>% dplyr::filter(NES_direction=="Negative"& traitgroup!="Other Gene Expression Change"& traitgroup!="Other/Unspecified") %>%
  dplyr::select(traitgroup, beta_direction, rsid, p , trait, NES_direction) %>%
  unique()
temp_full= df %>% dplyr::filter(traitgroup!="Other Gene Expression Change"& traitgroup!="Other/Unspecified")%>%
  dplyr::select(traitgroup, beta_direction, rsid, p , trait, NES_direction) %>%
  unique()

E2 = ggplot()+ #Plot of Traitgroup vs SNP Count
  geom_bar(data= temp_pos, aes(x=traitgroup, fill=beta_direction, y=after_stat(count)/nrow(temp_full)),  position = position_dodge2(width = 1, preserve = "single"))+
  geom_bar(data= temp_neg, aes(x=traitgroup, fill=beta_direction, y=(after_stat(count)/nrow(temp_full))*-1), position = position_dodge2(width = 1, preserve = "single"))+##  alpha is transparancy from 0-1
    geom_hline(yintercept=0, color ="gray70")+
  scale_fill_manual(values = colors_man) +
  labs(title= paste0("Number of STAs per Traitgroup Relative to Total Number of Traitgroups in this Dataframe") , x= "Trait Group", y= paste("Relative Number of STAs (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 60, hjust =1, size = 8),
...
The overview of all snps relative to traitgroups:
INCOMPLETE MAY25 - SEEMS TO BREAK WITH ENSEMBL ASSIGNING UNKNOWN GENE NAMES -COMMENTED OUT TO BE FIXED
```{r NO CUTOFF Trait Groups Effected by SNPs that Change in Expression of YOUR_GENE_NAME on Traits by logp}
temp = df[ which(is.na(df$tissue_group)==F),]
temp$beta_direction = factor(temp$beta_direction, levels=c("Positive", "Negative"))
...
temp = temp %>% dplyr::select(trait,traitgroup,logp_DIR,beta_direction,rsid) %>%
  unique()
test <- temp %>% dplyr::filter(grepl("ENSG",trait))
# if(nrow(temp)>0&nrow(test)>0){
                                     ...
                                     ifelse(gene_and_reg$EI== "Promoter Flanking Region", "#ABEBC6",
                                             ifelse(gene_and_reg$EI== "CTCF", "#C39BD3",
                                                     ifelse(gene_and_reg$EI== "TF binding", "#E67E22",
                                                             ifelse(gene_and_reg$EI== "Promoter", "#1ABC9C",
                                                                     ifelse(gene_and_reg$EI== "Exon", "#2C3E50",
                                                                           ifelse(gene_and_reg$EI== "Intron", "#56B4E9",                                                                                   ifelse(gene_and_reg$EI== "EMAR","#8f3e95","FAIL")))))))))
...
df_for_circle$color = ifelse( df_for_circle$reg_region== "Enhancer", "#196F3D",
                             ifelse(df_for_circle$reg_region== "Open chromatin", "#AAB7B8",
                                     ifelse(df_for_circle$reg_region== "Promoter Flanking Region", "#ABEBC6",
                                             ifelse(df_for_circle$reg_region== "CTCF", "#C39BD3",
...
####5. (A4) Graphs of where the SNPS are: (quick look before circos)
...
Unique_snps$reg_region = factor(Unique_snps$reg_region,levels = c("CTCF", "EMAR","Enhancer","Open chromatin","TF binding", "Promoter", "Promoter Flanking Region", "None"))
...
# Shiny app INCOMPLETE NEED TO FIX SHINY READING 10 as 2 11 as 3 etc 
```{r}
...
# Code snippets for deveolper modding:
```{r}
## MAKE A SHORTER QUER FOR PHEWAS CODE SNIPPET - SHORTER THAN RE RUNNING IN ENTIRETY#####
# a =final_df %>% 
#   separate_wider_delim(id,delim="-",names=c("id_short","id_short2","remove"), too_many = "merge")%>%
#   dplyr::select(id_short,id_short2) %>%
#   unite(id,id_short,id_short2,sep="-",remove=T)%>%
#   unique()
# batch_for_phewas = batch_for_phewas[!batch_for_phewas %in% a$id]

## Remerge with trait database#####
# Full_trait_database = read_csv(paste0(file_directory,"/Trait and Tissue Groups/Full_trait_database.csv"))
# Full_trait_database=unique(Full_trait_database)
# df =df %>% dplyr::select(-c(traitgroup,Subcat,Alt_Subcat))
# df= left_join(df, Full_trait_database)
# df=unique(df)
```
...
# Improvement ideas
Work out how to incude ieugwas token in the script: PERHAPS, make an empty .txt file in the directory, save token as that. read it in the code
vectorise the LD section instead of endless for loops
splice variants
gnomad re implementation
web interface - Shiny app
Progress saving for batched version of phewas()
community interface for recategorising the database
formalise into a function for github release (HUGE JOB)
REORDER the circos plot so that it goes in order of largest segment to smallest in both tissue and traits!

# Common Errors for compilation into FAQs / app troubleshooting
Ensembl Mirror DOWN "Possible SSL connectivity problems detected. - Error in .chooseEnsemblMirror(mirror = mirror, http_config = http_config) : 
  Unable to query any Ensembl site" - tends to be a connection issue, wifi disconnected etc but could do with a link to the server check site
