...
if(Pre_phewas_test==F){
  ...
  if(exists("Gtex_1")==T){
    ...
    # Replace gene symbol with GENCODE ID Versioned by GTEx
    api_url=paste0("https://gtexportal.org/api/v2/reference/gene?geneId=",gene_name)
    geneID = GET(url=api_url)
    geneID <- content(geneID, as = "text")
    geneID <- fromJSON(geneID)
    GENCODEID=geneID$data$gencodeId
    
    #New Version :
    # GTEx API endpoint for single tissue eQTL data
    api_url <- paste0("https://gtexportal.org/api/v2/association/singleTissueEqtl?gencodeId=",GENCODEID,"&itemsPerPage=100000")
  ...
  # Scraping version
    ...
    if (status_code(response) == 200) {
      ...
      eqtl_data = fromJSON(api_url,)
      Gtex_1 = eqtl_data$data
    } else {
      # Print an error message if the request was not successful
      cat("Error for tissue", tissue, ":", status_code(response), "\n")
