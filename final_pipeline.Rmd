...
```{r,Phewas searching }
unique_Gtex_snps = df$rsid
for(n in 1:round(length(na.omit(unique_Gtex_snps))/25, digits=0)){
...
write_csv(test,paste0(file_directory,"/",gene_name,"_test/Gtex/",gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")) # reduced df
} else{
  df = read_csv(paste0(file_directory,"/",gene_name,"_test/Gtex/",gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL.csv"))
  df$test ="filler"
  write_csv(df,paste0(file_directory,"/",gene_name,"_test/Gtex/",gene_name,"_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv"))
}
...
Finally, you may want the exact set of plots I have used in the example data. So, here is a list of the file name of each of those plots. Run it all and then you can just search your file directory accordingly:
...
(E3) e1-Other Gene Expression Change only
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/A0-RAW_GTEXTissueGROUP-rsid_count_OVERVIEW.pdf"), width= 150, height=100, unit="mm")
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/A-number_of_SNP-trait_associations_per_tissue_group.pdf"), width= 150, height=100, unit="mm")
...
#### C - "correlation_of_traits_to_total_possible_traits" Correlation between nmuber of SNPs in each trait group compared to trait database category size - Checking the pattern isn't just beecause of this:
...
Correlation_tg = Correlation + annotate(geom="text",  x=75, y=10000, label = paste0("r2 = ", round(r2[1],4)),
              fill="black")
Correlation_tg



ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/TG_dfcount_cor_leg.pdf"), width= 200, height=100, unit="mm")

Correlation_tg+theme(legend.position = "none")
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/C-number_of_tissues_each_SNP_affects_expression_in.pdf"), width= 60, unit="mm")
```
...
SNP_by_traitgroupe0
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/D-number_of_SNP-trait_associations_per_traitgroup.pdf"), width=200,height=120, unit="mm")
...
SNP_by_traitgroupe0REL= ggplot(summary_of_df, aes(x=reorder(traitgroup, -rel.traitgroup), y=rel.traitgroup, fill= traitgroup))+
 ...
 # geom_text(check_overlap = T,aes( y=rel.traitgroup, label=rel.traitgroup), vjust=-0.5)+
  ...
  theme(legend.position = "none",
    ...
    scale_x_discrete(expand = c(0.04, 0)) +
    scale_y_continuous( expand = expansion(mult = c(0, .1))) +
  labs(x = "Trait Group", y = "n(SNP-Trait Associations per Trait Group)/total n(SNP-Trait Associations)", title = "Number of SNP-Trait Associations in Each Traitgroup")
SNP_by_traitgroupe0REL
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/D-normalised.pdf"), width=200,height=120, unit="mm")
...
pvalvstraitgroupe1 = ggplot(temp, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  labs(title= paste0("Trait Groups Affected by SNPs that Change Expression of ",gene_name) , x= "Trait Group", y= "-log(p) x Direction of expression change")+ #labels
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/E1--logp_x_expression_change_per_traitgroup.pdf"), width=200, height =100 , units="mm")
```
...
pvalvstraitgroupNO_O_e2 = ggplot(temp, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + 
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/e2-e1_no_Other_or_OGEC_LEGEND.pdf"), width=200, height =100 , units="mm")

pvalvstraitgroupNO_O_e2+theme(legend.position = "none")

ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/E2-e1_no_Other_or_OGEC.pdf"), width=200, height =100 , units="mm")
```
...
OGECe3 = ggplot(temp, aes(x=trait, y=logp_DIR, color=beta_direction)) + 
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + 
...
filename=paste0(file_directory,"/",gene_name,"_test/Final_plots/E3-e1_OGEC_only.pdf")
...
Oe3 = ggplot(temp, aes(x=trait, y=logp_DIR, color=beta_direction)) + 
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + 
  labs( x= "Genes", y= "-log(p) * Direction")+ 
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
...
filename=paste0(file_directory,"/",gene_name,"_test/Final_plots/E3-e1_Other_only.pdf")
...
#### E4 Top n categories after other and OGEC:
...
for(i in 3:nrow(number_of_traitgroups)-2){
  ...
  pvsTrait_bygroup = ggplot(temp, aes(x=trait, y=logp_DIR, color=beta_direction)) + 
    ...
    labs(title= paste0("Traits in ",summary_of_df_no_OGEC$traitgroup[i]," Affected by SNPs that Change in Expression of ", gene_name) , x= "Traits", y= "-log(p) x Direction")+ #labels
    geom_hline(aes(yintercept=0,), alpha=0.3)+# faint line at 0
...
filename=paste0(file_directory,"/",gene_name,"_test/Final_plots/E4-Number_", i,"_traitgroup.pdf")
  if(for_label>150){
    ggsave(filename, height=560, width=230, unit="mm")
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/F1-nSNP-trait_associations_per_traitgroup_by_total.pdf"), width=200, height =100 , units="mm")



```

#### F2 Proportional (WITHOUT other gene expression change (OGEC))
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/F2-F1_no_OGEC.pdf"), width=200, height =100 , units="mm")
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/G-number_of_snps_per_regulatory_region.pdf"))
```
...
circos.clear()
pdf("H-Tissue_trait_overlap.pdf")
