...
You'll need to assign a gene_name, file_directory and LD_token in this block of code, making sure each is within "" as in the examples. 
  ...
  4. Install all the packages - this should be automatic but is the NUMBER 1 source of error. It drives me mad and is 99% of the time due to admin rights on IT managed computers not playing nicely with R installing non-CRAN but very much verified packages. I'm happy to sort this manually for you if you run into an issue, just ping me an email.
...
#windows: file_directory = "C:/Users/hy14913/OneDrive - University of Bristol/Desktop/May23_test"
...
#e.g LD_token ="f0d7bc798f5c"generic_token
...
write_csv(egene ,paste0(file_directory,"/eGene/FINAL_Gtex_egene.csv"))
  ...
  dir.create(paste0(file_directory,"/",gene_name,"_test/Ensembl/All_Exon_Options"))
  

  dir.create(paste0(file_directory,"/", gene_name,"_test/Gtex/Plots"))
  dir.create(paste0(file_directory,"/", gene_name,"_test/Gtex/Final Dataframes"))
...
for(n in 1:round(length(unique_Gtex_snps)/25, digits=0)){
...
(E0) e0-ntraitgroup_associations and e0-RELntraitgroup_associations (raw and relative respectively)
(E1) pvalvstraitgroup_LOG
...
(F1) COUNTvstraitgroup_REL_overview_SEATLE#
(F2) f2COUNTvstraitgroup_REL_overview_SEATLE_NOOGEC#
...
df = df %>% separate(id,into=c("remove1","remove2","remove3","irntvsraw" ), remove=F) %>%
...
```
#### A - TissueGROUP-rsid_count_OVERVIEW ORIGINAL GTEX
...
SNPcountperTissuegroup = ggplot(test, aes(x=tissue_group,y=count, fill=NES_direction)) + 
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/A-RAW_GTEXTissueGROUP-rsid_count_OVERVIEW.pdf"), width= 150, height=100, unit="mm")
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/TG_dfcount_cor_leg.pdf"), width= 100, height=100, unit="mm")
...
#Alternate version, relative to tissuegroup size#####
...
SNP_by_traitgroupe0REL= ggplot(summary_of_df, aes(x=reorder(traitgroup, -rel.traitgroup), y=rel.traitgroup, fill= traitgroup))+
  geom_col()+
 # geom_text(check_overlap = T,aes( y=rel.traitgroup, label=rel.traitgroup), vjust=-0.5)+
  ...
  theme(legend.position = "none",
    ...
    scale_y_continuous( expand = expansion(mult = c(0, .1))) +
  labs(x = "Trait Group", y = "Number of SNP-Trait Associations", title = "Number of SNP-Trait Associations in Each Traitgroup")
SNP_by_traitgroupe0REL
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/e0-RELntraitgroup_associations.pdf"), width=200,height=120, unit="mm")
...
ggsave(paste0(file_directory,"/",gene_name,"_test/Final_plots/e2-LEGEND.pdf"), width=200, height =100 , units="mm")
```
...
f1 = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  theme(axis.text.x = element_text(angle = 60, hjust =1, size = 8),
        ...
        panel.background = element_rect(fill="white"),
        #legend.position = "none"
        )  # setting up x axis labels so they don't overlap
...
# INCOMPLETE APRIL - needs new file directory
...
# Figure of all my favourite plots:
 ...
 INCOMPLETE APRIL 23
 Need to sort out the shiny implementation properly as well as add in error jumping for the circos plots if they're not playing the game.
```{r}
...
df <- df[, col_order]
df = unique(df)
write_csv(df,paste0(file_directory,"/", gene_name,"_test/Final_data/final_df_reordered.csv"))
...
```{r Plotting the raw Gtex egene data against Tissue line graph, include=FALSE}
