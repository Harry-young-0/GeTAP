...
---
TESTING: Your gene name is GPX1. If this still reads "your gene name" not a specific gene you have chosen (for example KEAP1), Please try the ctrl-f step again 
...
I would suggest setting up a folder to run all of this into, copying it over to backup files as you go, here I've named in GPX1_test:

e.g, use the following code in every chunk:
```{r}
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
```

Firstly we read in the dataframe of interest and do some initial clean-up steps:

To read in the dataframe first search your gene of interest into Gtex portal. Then click significant single-tissue eQTL and download the csv. Rename the file Gtex_GPX1_START.
```{r Initial load and clean}

setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
Gtex_GPX1 = read_csv("Gtex_GPX1_START.csv")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_excel_csv(df, "GPX1_Gtex_cleaned.csv")
...
unique_GPX1_Gtex_snps = unique(df$rsid)
# setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Quantitative and Qualitative SNP lists")
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write.table(unique_GPX1_Gtex_snps, "unique_GPX1_Gtex_snps.csv", sep="\t", row.names=FALSE, col.names=FALSE) # saving the file
```
...
First step is to search your list of snps (unique_GPX1_Gtex_snps) into LDMatrix to get an interactive plot of linkage disequilibrium in your snp set. https://ldlink.nci.nih.gov/?tab=ldmatrix 
...
if(length(unique_GPX1_Gtex_snps)<700){

setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/LDMatrix")
GPX1_r2 = LDmatrix(unique_GPX1_Gtex_snps, pop = "EUR", r2d = "r2", token ="f0d7bc798f5c")
write_csv(GPX1_r2, "GPX1_r2.csv")



# open up the .csv file from the website as tab-deliminated 
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/LDMatrix")
GPX1_r2 = read_csv("GPX1_r2.csv")
head(GPX1_r2)


for(n in 1:ncol(GPX1_r2)){#         LOOP to create a df of NA and values above 0.5 - CHANGE THIS CUTOFF TO WHAT YOU WANT
  if(n==1){ # First column is the rsid so skip it using 'next'
    next
  }
  else if(n==2){linked0.5 = GPX1_r2[which(GPX1_r2[,n]>=0.5),] # For the first column setup a new df to write to, called 'linked0.5'
    linked0.5 = linked0.5[,c(1,n)]
  }
  else{
    linked0.5_temp =GPX1_r2[which(GPX1_r2[,n]>=0.5),] # Each loop you are making a temporary file 'linked0.5_temp', 
...
for(n in 1:ncol(GPX1_r2)){#         LOOP to create a df of NA and values above 0.5 - CHANGE THIS CUTOFF TO WHAT YOU WANT
  ...
  else if(n==2){linked0.75 = GPX1_r2[which(GPX1_r2[,n]>=0.75),] # For the first column setup a new df to write to, called 'linked0.75'
    linked0.75 = linked0.75[,c(1,n)]
  }
  else{
    linked0.75_temp =GPX1_r2[which(GPX1_r2[,n]>=0.75),] # Each loop you are making a temporary file 'linked0.75_temp', 
...
rs_overlap_r20.5 = vector(mode="list", length = nrow(GPX1_r2)) # set up a dictionary style list
names(rs_overlap_r20.5) = names(GPX1_r2[,2:ncol(GPX1_r2)]) # add in the names of the snps (rsids)
...
rs_overlap_r20.75 = vector(mode="list", length = nrow(GPX1_r2)) # set up a dictionary style list
names(rs_overlap_r20.75) = names(GPX1_r2[,2:ncol(GPX1_r2)]) # add in the names of the snps (rsids)
...
rm(df, GPX1_r2, Gtex_GPX1, linked_test0.5, linked_test0.75, linked0.5, linked0.75, temp, temp0.5D_2, temp0.75D_2, unique_snps, i,n,test)
##Save the environment
save.image("rs_overlap_ALL_GPX1.RData")
...
if(length(unique_GPX1_Gtex_snps)>=1000){
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/LDMatrix")

 library(LDlinkR)
 GPX1_LIMITED_SNP_LIST = SNPclip(
   unique_GPX1_Gtex_snps,
   pop = "EUR",
   r2_threshold = "0.5",
   maf_threshold = "0.01",
  token = "f0d7bc798f5c" )
...
GPX1_LIMITED_SNP_LIST = GPX1_LIMITED_SNP_LIST[grep("Variant kept.", GPX1_LIMITED_SNP_LIST$Details),  ] # selects the ones that passed the test
GPX1_LIMITED_SNP_LIST = as.data.frame(GPX1_LIMITED_SNP_LIST[,1])
names(GPX1_LIMITED_SNP_LIST) = "rsid"

write_csv(GPX1_LIMITED_SNP_LIST, "GPX1_LIMITED_SNP_LIST.csv")
unique_GPX1_Gtex_snps = read_csv("GPX1_LIMITED_SNP_LIST.csv")# , col_names=F
unique_GPX1_Gtex_snps = unique_GPX1_Gtex_snps$rsid
}
```
...
save this list as GPX1_LIMITED_SNP_LIST.csv in GPX1_test/Gtex
...
IMPORTANT: Here I have left it as the unique_GPX1_gtex_snps (unique snp list) but you can also use your LIMITED_snp_list (GPX1_LIMITED_SNP_LIST) from step 2.3 IF YOU USED IT. 
...
# change out list of snps from unique_GPX1_Gtex_snps to LIMITED_SNP_LIST here depending on choice above
 Gtex_PHEWAS = phewas(unique_GPX1_Gtex_snps, pval = 1e-04) # Adjustable p-value here #
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_excel_csv(Gtex_PHEWAS, "GPX1_GtextoPHEWAS_RAW.csv")
}
```
The result of this is seen below with head(GPX1_GtextoPHEWAS_RAW).
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
GPX1_GtextoPHEWAS_RAW = read_csv("GPX1_GtextoPHEWAS_RAW.csv") # load in Phewas output
GPX1_Gtex_cleaned = read_csv("GPX1_Gtex_cleaned.csv") #  load in Gtex input

GPX1_GtextoPHEWAS_RAW = GPX1_GtextoPHEWAS_RAW %>%
...
# NOT NECERSARY BUT MAKES IT EASIER TO LOOK AT !
 ...
 GPX1_Gtex_cleaned = GPX1_Gtex_cleaned[,c( "Gencode Id","Gene Symbol","rsid","a","b", "NES","|NES|","Tissue", "P-Value", "Variant Id","chr_gtex", "position_gtex", "build_gtex")]
 head(GPX1_Gtex_cleaned)
 GPX1_GtextoPHEWAS_RAW = GPX1_GtextoPHEWAS_RAW[,c("rsid", "ea","nea","trait","beta", "p","se","n","eaf","chr_phewas","position_phewas", "id")] # "traitgroup",
 head(GPX1_GtextoPHEWAS_RAW)
```
...
test = merge(GPX1_Gtex_cleaned, GPX1_GtextoPHEWAS_RAW, all=T, by = "rsid")
# OR
test2 = full_join(GPX1_Gtex_cleaned, GPX1_GtextoPHEWAS_RAW, by = "rsid")
...
# test_rsid = PS_GPX1[which(PS_GPX1$rsid =="rs78616393"),]
# test_rsid = Gtex_GPX1[which(Gtex_GPX1$rsid =="rs78616393"),]

rm(test2) # cleanup removing the extra table 

  
#setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Cross-Reference/GtextoPheWAS-Sorted/GPX1")
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_excel_csv(test, "GPX1_merge_GTEX_PHEWAS.csv")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df = read_csv("GPX1_merge_GTEX_PHEWAS.csv")
...
write_excel_csv(df, "GPX1_merge_GTEX_PHEWAS-betacor.csv")
```
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df = read_csv("GPX1_merge_GTEX_PHEWAS-betacor.csv")
...
#setwd("~/OneDrive - University of Bristol/PhD/Epidemiology/Cross-Reference/GtextoPheWAS-Sorted/GPX1")
write_excel_csv(df, "GPX1_merge_GTEX_PHEWAS_betacor_ambig.csv")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df = read_csv( "GPX1_merge_GTEX_PHEWAS_betacor_ambig.csv")
...
# save unorganised
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_excel_csv(df, "GPX1_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv")
```
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df = read_csv("GPX1_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv")
...
#And finally save it:
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_csv(df, "GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
```
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
unique_GPX1_Gtex_snps= read_tsv("unique_GPX1_Gtex_snps.csv", col_names = FALSE) # read the original unique snps file
unique_GPX1_Gtex_snps = unique_GPX1_Gtex_snps$X1
if(length(unique_GPX1_Gtex_snps)<700){
rm(list=ls())
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
load("rs_overlap_ALL_GPX1.RData")  # specifically the dataframe with ambiguous data removed
...
df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
backup =df
...
write_csv(test2, "GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_linkagetempr20.5.csv")# full dataframe but with the "test" column 
...
write_csv(test, "GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv") # reduced df
} else{
  df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
  df$test ="filler"
  write_csv(df, "GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
}
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/Gtex_egene")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/Gtex_egene") # SAVE it in dedicated plotting folder
...
## our df plotting
First step is to make sure that you are using the correctly filtered dataframe for no ambiguous or ukb-a data by loading in the ukaORambig_FINAL OR linkage removed set:  GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv dataframe as df:
```{r Reading in the FINAL df}
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
# df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL.csv")
df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
```
...
First, download the overview of GPX1 expression in the body from the Gtex search and save this.
...
Gtex_GPX1_START = read_csv("Gtex_GPX1_START.csv")

NESvsTiss_full = ggplot(Gtex_GPX1_START, aes(x=Tissue, y=NES, color = `P-Value`)) + #Plot of SNPs NES vs beta from original dataset
  #geom_jitter(aes(color=NES)) +
  geom_point(position = position_jitterdodge())+ # point plot with jitter to avoid overlap
  scale_colour_viridis_c(direction=-1) + # colourblind friendly palette
  labs(title= "All GPX1 Expression Changes", x= "Tissue", y= "NES")+ # Lables
  geom_hline(aes(yintercept=0,))+ #  0 line
  theme(axis.text.x = element_text(angle = 70, hjust =1))  # text adjust
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/NES_vs_tissue_and_overview") # save in this dedicated folder
...
NESvsTiss = ggplot(eQTL_from_Phewas, aes(x=Tissue, y=NES, color = `P-Value`)) + #Plot of SNPs NES vs beta
  ...
  labs(title= "GPX1 Expression Changes Occuring in PheWAS Search", x= "Tissue", y= "NES")+
  geom_hline(aes(yintercept=0,))+
  theme(axis.text.x = element_text(angle = 70, hjust =1)) +
  geom_vline(aes(xintercept=0,))
NESvsTiss

setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/NES_vs_tissue_and_overview")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots") #SAVE it
...
Count_plot = ggplot(df, aes(x=traitgroup, y=tissue_group)) + #Plot of Tissue vs trait for overlap
  geom_count()+
  scale_color_viridis_c()+
  labs(title= "Overall GPX1 Correlations", paste("Overall Count = ", nrow(df)), x ="Trait Group", y = "Tissue Group")+  #CHANGETHIS
  theme(axis.text.x = element_text(angle = 45, hjust =1))
Count_plot


setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/Tissue_Group_and_Trait_Group_overlap") #SAVE it
...
Count_facet = ggplot(df, aes(x=traitgroup, y=tissue_group)) + #Plot of Tissue vs trait for overlap
  geom_count()+
  scale_color_viridis_c()+
  labs(title= "GPX1 Correlations by Change in Expression", caption = paste("Overall Count = ", nrow(df), "; Positive Count = ", nrow(df[which(df$NES>0),]), "; Negative Count = ", nrow(df[which(df$NES<0),])), x ="Trait Group", y = "Tissue Group")+  #CHANGETHIS
  theme(axis.text.x = element_text(angle = 45, hjust =1))+ 
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_csv(LD_Gtex, "GPX1_LDRED_Gtex.csv")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/snp_against_Tissue") #SAVE it
...
Full_tissue_database = read_csv("Full_tissue_database.csv")
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
Gtex = read_csv("GPX1_Gtex_cleaned.csv")
...
write_csv(Gtex, "GPX1_Gtex_cleaned.csv") 
...
df_GPX1_RED = df %>% dplyr::select(Tissue, tissue_group, `P-Value`) 
df_GPX1_RED = unique(df_GPX1_RED)
...
df_GPX1_forcomp = df_GPX1_RED %>% group_by(Tissue, tissue_group) %>% summarise(count = length(Tissue[!is.na(Tissue)]))

comparison_RNA = left_join(df_GPX1_forcomp, Gtex_RNA_forcomp, by=c("Tissue", "tissue_group"))
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/snp_against_Tissue") # SAVE it in dedicated plotting folder
...
```{r NO CUTOFF Trait Groups Effected by SNPs that Change in Expression of GPX1 on Traits by logp}
...
pvsTrait = ggplot(df, aes(x=traitgroup, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= "-log(p)")+ #labels
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/p vs traitgroup split Expression direction") #SAVE it
...
pvsTrait = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= paste("Number of SNP-Trait \nAssociations"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+  # setting up x axis labels so they don't overlap
...
pvsTrait = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+  # setting up x axis labels so they don't overlap
  #facet_grid(cols = vars(NES_direction)) # facet splits the plot based on a variable, in this case NES direction
  annotate("label",
           x=c(length(unique(df$traitgroup))-0.7,length(unique(df$traitgroup))-0.7),
...
```{r Trait Groups Effected by SNPs that Change in Expression of GPX1 on Traits}
...
pvsTraitCUTOFF = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  #ylim(-0.175,0.175)+
  scale_fill_viridis_d(direction=-1) + # I like this palette but you can remove it or change it 
  labs(title= "Trait Groups Effected by SNPs that Change in Expression of GPX1 (p<5e-8)" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+  # setting up x axis labels so they don't overlap
  #facet_grid(cols = vars(NES_direction)) # facet splits the plot based on a variable, in this case NES direction
  annotate("label",
...
pvsTrait_bytraitgroup = ggplot(df, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  geom_point(alpha=0.6, position = position_jitterdodge())+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Traits Effected by SNPs that Change in Expression of GPX1" , x= "Trait by Traitgroup", y= "-log(p)")+ #labels
  geom_hline(aes(yintercept=0,), alpha=0.3)+ # faint line at 0
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/p vs trait split by trait group and Expression direction") #SAVE it
ggsave("pvsTrait_bytraitgroup.pdf", width = 30, height =15)# Actually setting some dimensions due to long names
```
With cutoff:

```{r Traits Effected by SNPs that Change in Expression of GPX1}
...
pvsTrait_byINFL = ggplot(subcat, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Inflammation Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= "-log(p)")+ #labels
...
pvsTrait_bybody = ggplot(subcat, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  geom_point(position = position_jitterdodge(), size =1)+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Body Fat/Size Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(yintercept=0, alpha=0.3)+# faint line at 0
  geom_hline(yintercept=4, alpha=0.3, color="red")+
  geom_hline(yintercept=-4, alpha=0.3, color="red")+
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
...
pvsTrait_bybody = ggplot() + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  labs(title= "Body Fat/Size Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= "Relative Count")+ #labels
  geom_hline(yintercept=0, alpha=0.3)+# faint line at 0
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
  facet_grid(cols = vars(Subcat), scales="free", space="free_x") # facet splits the plot based on a variable, in this case NES direction
  # annotate("label",
...
pvsTrait_bykidney = ggplot(subcat, aes(x=trait, y=logp_DIR, color=beta_direction)) + #Plot of Traitgroup vs p (P-Value is the Gtex P not PS p)
  ...
  geom_point(position = position_jitterdodge(), size =1)+ #  alpha is transparancy from 0-1
  scale_colour_viridis_d(direction=-1, ) + # I like this palette but you can remove it or change it - green/blue option begin=0.2, end=0.7
  labs(title= "Kidney Trait Groups Effected by SNPs that Change in Expression of GPX1" , x= "Trait Group", y= "-log(p)")+ #labels
  geom_hline(yintercept=0, alpha=0.3)+# faint line at 0
  geom_hline(yintercept=4, alpha=0.3, color="red")+
  geom_hline(yintercept=-4, alpha=0.3, color="red")+
  theme(axis.text.x = element_text(angle = 45, hjust =1)) + # setting up x axis labels so they don't overlap
...
```{r Number of SNPs that Change Expression of GPX1 and Effect a Trait, fig.width=30,fig.height=30}
...
num_of_SNPs_by_beta = ggplot()+ #Plot of Traitgroup vs SNP Count
  ...
  labs(title= "Number of SNPs that Change in Expression of GPX1 and Effect a Trait" , x= "Trait Group", y= paste("Relative Number of SNP-Trait \nAssociations (n/Total)"))+ #labels
  theme(axis.text.x = element_text(angle = 45, hjust =1))+ # setting up x axis labels so they don't overlap
  facet_grid(cols = vars(traitgroup), scales = "free_x", switch = 'x') 
...
First you have to look up the exon table and regulatory features from ensembl. Then bind it to the new merged df and the original GPX1_Gtex_cleaned df. This step is a little more manual. Unfortunately gene to gene it's hard to write a consistent bit of code for finding introns and exons and cleaning up the df to be what you want it to be. Maybe I can come back here in the final tidying steps! 
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Ensembl/All_Exon_Options")
...
GPX1_structure =bind_rows(myfiles, .id="id") # merges the list of dataframes into one dataframe

GPX1_structure = GPX1_structure[!grepl("stream", GPX1_structure$`Exon / Intron`),] # remove the 5' and 3' up/downstream regions
GPX1_structure$EI <- !!rowSums(sapply(GPX1_structure, grepl, pattern = "ENSE"))
GPX1_structure$EI = if_else(GPX1_structure$EI ==TRUE, "Exon", "Intron")

# Finally just making a simple to use mini version of the main table of exons/introns
GPX1_structure_FIN = GPX1_structure %>% dplyr::select( Start, End, EI)
GPX1_structure_FIN = unique(GPX1_structure_FIN)


setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Ensembl")
write_csv(GPX1_structure_FIN, "GPX1_structure_FIN.csv")
write_csv(GPX1_structure, "GPX1_structure_FULL.csv")
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Ensembl")
GPX1_structure_FIN =read_csv("GPX1_structure_FIN.csv")
GPX1_structure =read_csv( "GPX1_structure_FULL.csv")
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
Gtex = read_csv("GPX1_Gtex_cleaned.csv")
...
limitsSTART = GPX1_structure[which.max(GPX1_structure$Start),]
limitsEND = GPX1_structure[which.min(GPX1_structure$End),]
limits =rbind(limitsSTART,limitsEND)

exons = GPX1_structure[which(GPX1_structure$EI=="Exon"),] # select just exon rows
...
df$Intron_or_Exon = "Intron" # confusingly GPX1 runs backwards so all the signs are flipped to make sense : START>POS>END
for( n in 1:nrow(df)){
...
Gtex$Intron_or_Exon = "Intron" # confusingly GPX1 runs backwards so all the signs are flipped to make sense : START>POS>END
for( n in 1:nrow(Gtex)){
  for(i in 1:nrow(exons)){
...
chr = df$chr_phewas[1]
GPX1_region_st = min(Gtex$position_gtex-10000) # 9640000
GPX1_region_fin = max(Gtex$position_gtex+10000) #  11400000
filter_values=list(chr,GPX1_region_st,GPX1_region_fin)
temp = getBM(attributes=c("chromosome_name", "chromosome_start", "chromosome_end", "feature_type_name"), 
      filters = c("chromosome_name", "start", "end"),
      values = filter_values, 
      mart = ensembl)

write_csv(temp, "GPX1_full_REG_region.csv")
```
Now we assign those reg features to df and gtex dataframes
```{r}
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Ensembl")
reg_feat = read_csv("GPX1_full_REG_region.csv") # from biomart from ensembl - 
# setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
# df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv") # from before
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df_merge = read_csv("GPX1_merge_GTEX_PHEWAS_betacor_ambig_grouped.csv")
...
write_csv(df_merge, "GPX1_merge_GTEX_PHEWAS_betacor_ambig_grouped_reg.csv")
# I'm just overiding this file with the new one for simplicity but you can make a new file if you see fit
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
write_csv(df, "GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
write_csv(Gtex, "GPX1_Gtex_cleaned_REG.csv")
...
DO THESE NEXT FEW STEPS for GPX1 KEAP1 ORMDL3 and ATP5f1
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
df_merge = read_csv("GPX1_merge_GTEX_PHEWAS_betacor_ambig_grouped_reg.csv")
Gtex = read_csv("GPX1_Gtex_cleaned_REG.csv")
df = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Ensembl")
reg_feat = read_csv("GPX1_full_REG_region.csv") 
GPX1_rectangles = read_csv("GPX1_structure_FIN.csv") # from before
```

Now, importantly, both for the linear and curved gene diagram the leg work remains the same. 
* Define the regions by having start, end and feature columns (as in GPX1_rectangles). 
...
First, we need to take the regulatory feature dataframe (downloaded form ensembl BioMART - see start of section) and clean it to look like GPX1_rectangles
```{r Setting up reg feature dataframe }
# Now simplifying the regulatroy region page down
reg_feat_for_bind = reg_feat[,2:4]
# sorting the names to be the same as the GPX1_rectangles 
...
gene_and_reg = rbind(reg_feat_for_bind, GPX1_rectangles)

setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Ensembl") # saving in the Ensembl folder
write_csv(gene_and_reg, "GPX1_and_reg_feat_FOR_GENE_DIAGRAM.csv")
...
gene_structure_G = ggplot()+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= "GPX1 Gene Region Diagram (Gtex SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gene_diagram_output")
...
gene_structure_RED = ggplot()+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= "GPX1 Gene Region Diagram (Gtex after Merge SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
gene_structure_RED = ggplot()+
  ...
  theme(panel.grid.major.x = element_line(size=0.2),
        ...
        panel.background = element_rect(fill = "white",
                                        colour = "white"))+
  labs(title= "GPX1 Gene Region Diagram (Gtex after Merge and LD removal SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
gene_structure_RED_P = ggplot()+
  ...
  labs(title= "GPX1 Gene Region Diagram (Gtex-PheWAS SNPs)" , x= "Position", y= "-log(p)",caption = paste("Overall number of SNPs =", Unique_snps))+
  scale_fill_manual(values=cols)+
  scale_color_manual(values=cols)+
  scale_x_reverse()
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex/Plots/REG_region_counts") #SAVE it
...
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Circos")
...
library(tidyverse)
setwd("C:/Users/Harry Young/Documents/Local Wokr/GPX1_test/Gtex")
Gtex_RED = read_csv("GPX1_Gtex_cleaned_REG.csv")
df_final = read_csv("GPX1_gtexPHEWAS_no_ukaORAmbig_FINAL_LINKEDREMOVEDr20.5.csv")
df =df_final
...
# COMMON ERRORS
'Error: 'GPX1_Dprime.csv' does not exist' - You need to run this chunk (CHUNK 7 - r Automated r2 and d prime START dataset, eval= FALSE, include=FALSE) of code seperately using the little play symbol. I've done this because it can be a lengthy step and is optional (only use if you have less than 999 uniques SNPs)
